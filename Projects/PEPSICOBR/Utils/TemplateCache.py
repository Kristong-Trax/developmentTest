...
class PEPSICOBRTemplateAsCache:

    JSON = 'Json'
    DATA_FRAME = 'DataFrame'
    EXCEL = 'Excel'

    def __init__(self, project, templates_path_in_cloud, template_type=JSON,
    ...
    def amz_conn(self):
        if not hasattr(self, '_amz_conn'):
            self._amz_conn = self._amz_conn = BucketConnector(BUCKET)
    ...
    def convert_excel_file(self, file_path):
        if self.template_type == self.DATA_FRAME:
            data = parse_template(file_path)
        elif self.template_type == self.JSON:
            data = PEPSICOBRGENERALToolBox.get_json_data(file_path)
        else:
    ...
    def upload_templates(self, templates_path, templates_names, align_with_existing=True, immediate_change=False):
        ...
        for name in existing_templates.keys():
            self.amz_conn.save_file(new_folder_path, name, existing_templates[name])
    ...
    def get_template(self, name):
        ...
        """
        with open(os.path.join(self.cache_path, name), 'rb') as f:
            if self.template_type == self.DATA_FRAME:
                data = pd.read_json(f)
            elif self.template_type == self.JSON:
                data = json.load(f)
