...
class SceneToolBox:
    ...
    def calculate_facings_per_sku(self, location_type_1, kpi_name):
        if location_type_1:
            result_df = self.count_facings_by_scenes(self.scif, {'location_type':(self.tools.INCLUDE_FILTER),
                                                                "product_type":"SKU"})[['product_fk', 'facings']]
        else:
            result_df = self.count_facings_by_scenes(self.scif, {'location_type':(self.tools.EXCLUDE_FILTER),
                                                                "product_type":"SKU"})[['product_fk', 'facings']]
