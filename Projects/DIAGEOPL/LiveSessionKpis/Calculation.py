...
class CalculateKpi(LiveSessionBaseClass):
    ...
    def __init__(self, data_provider, output):
        ...
        self.assortment = LiveAssortmentCalculation(data_provider, live_assortment=False, wish_list=True)
    ...
    @log_runtime('flat_live_assortment', log_start=True)
    def calculate_session_live_kpi(self):
        """
        Main function of live project
        """
    ...
    def availability_by_assortment_calc(self):
        ...
        if lvl3_result.empty:
            ...
            return
        diageo_kpis = [self.get_kpi_fk(kpi) for kpi in self.DIAGEO_KPIS]
        lvl3_result = lvl3_result[lvl3_result.kpi_fk_lvl2.isin(diageo_kpis)]
        if lvl3_result.empty:
            Log.warning('Assortment is Empty for this session')
            return
