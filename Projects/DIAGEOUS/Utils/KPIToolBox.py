...
from Projects.DIAGEOUS.Utils.Const import DIAGEOUSConst
...
class DIAGEOUSToolBox:
    ...
    def main_calculation(self, *args, **kwargs):
        ...
        total_kpi_fk = self.common.get_kpi_fk_by_kpi_name(DIAGEOUSConst.DB_OFF_SCORE_TOTAL)
        segment_kpi_fk = self.common.get_kpi_fk_by_kpi_name(DIAGEOUSConst.DB_OFF_SCORE_SEGMENT)
        national_kpi_fk = self.common.get_kpi_fk_by_kpi_name(DIAGEOUSConst.DB_OFF_SCORE_NATIONAL)
        for i, kpi_line in self.templates[DIAGEOUSConst.KPIS_SHEET].iterrows():
            total_weighted_score, segment_weighted_score, national_weighted_score = self.calculate_set(kpi_line)
            if kpi_line[DIAGEOUSConst.KPI_GROUP]:
        ...
        self.common.write_to_db_result(
            ...
            identifier_result=self.common.get_dictionary(name=DIAGEOUSConst.TOTAL), score=total_store_score)
        self.common.write_to_db_result(
            fk=segment_kpi_fk, numerator_id=self.manufacturer_fk, result=segment_store_score,
            identifier_result=self.common.get_dictionary(name=DIAGEOUSConst.SEGMENT), score=segment_store_score)
        self.common.write_to_db_result(
            fk=national_kpi_fk, numerator_id=self.manufacturer_fk, result=national_store_score,
            identifier_result=self.common.get_dictionary(name=DIAGEOUSConst.NATIONAL), score=national_store_score)
    ...
    def calculate_assortment(self, scene_types, kpi_name, weight):
        ...
        if kpi_name == DIAGEOUSConst.POD:
            calculate_function = self.calculate_pod_sku
        elif kpi_name == DIAGEOUSConst.DISPLAY_BRAND:
            ...
            return 0, 0, 0
        total_level_fk = self.common.get_kpi_fk_by_kpi_name(DIAGEOUSConst.DB_OFF_NAMES[kpi_name][DIAGEOUSConst.TOTAL])
        relevant_assortment = self.assortment_products[self.assortment_products['kpi_fk_lvl2'] == total_level_fk]
        all_results = pd.DataFrame(columns=DIAGEOUSConst.COLUMNS_FOR_PRODUCT)
    ...
    def calculate_extras(self, relevant_assortment):
        ...
        :return:
        """
        sku_kpi_fk = self.common.get_kpi_fk_by_kpi_name(DIAGEOUSConst.DB_OFF_NAMES[DIAGEOUSConst.POD][DIAGEOUSConst.SKU])
    ...
    def calculate_pod_sku(self, product_fk, relevant_scif):
        ...
        :return:
        """
        kpi_fk = self.common.get_kpi_fk_by_kpi_name(DIAGEOUSConst.DB_OFF_NAMES[DIAGEOUSConst.POD][DIAGEOUSConst.SKU])
        total_kpi_fk = self.common.get_kpi_fk_by_kpi_name(DIAGEOUSConst.DB_OFF_NAMES[DIAGEOUSConst.POD][DIAGEOUSConst.TOTAL])
        facings = relevant_scif[relevant_scif['product_fk'] == product_fk]['facings'].sum()
        if facings > 0:
            result, passed = DIAGEOUSConst.DISTRIBUTED, 1
        else:
            result, passed = DIAGEOUSConst.OOS, 0
        ...
        standard_type = "s"
        product_result = {DIAGEOUSConst.PRODUCT_FK: product_fk, DIAGEOUSConst.PASSED: passed,
                          DIAGEOUSConst.BRAND: brand, DIAGEOUSConst.SUB_BRAND: sub_brand, DIAGEOUSConst.STANDARD_TYPE: standard_type}
        return product_result

    def calculate_display_compliance_sku(self, product_fk, relevant_scif):
        kpi_fk = self.common.get_kpi_fk_by_kpi_name(DIAGEOUSConst.DB_OFF_NAMES[DIAGEOUSConst.DISPLAY_BRAND][DIAGEOUSConst.SKU])
        total_kpi_fk = self.common.get_kpi_fk_by_kpi_name(DIAGEOUSConst.DB_OFF_NAMES[DIAGEOUSConst.DISPLAY_BRAND][DIAGEOUSConst.TOTAL])
    ...
    def calculate_total_display_share(self, scene_types, weight, target):
        ...
        :return: total_result
        """
        total_kpi_fk = self.common.get_kpi_fk_by_kpi_name(DIAGEOUSConst.DB_OFF_NAMES[DIAGEOUSConst.DISPLAY_SHARE][DIAGEOUSConst.TOTAL])
        manufacturer_kpi_fk = self.common.get_kpi_fk_by_kpi_name(DIAGEOUSConst.DB_OFF_NAMES[
                                                                     DIAGEOUSConst.DISPLAY_SHARE][DIAGEOUSConst.MANUFACTURER])
        ...
        all_results = pd.DataFrame(columns=DIAGEOUSConst.COLUMNS_FOR_DISPLAY)
        ...
        den_res = all_results[DIAGEOUSConst.PASSED].sum()
        diageo_results, diageo_result = 0, 0
        for manufacturer in all_results[DIAGEOUSConst.MANUFACTURER].unique().tolist():
            num_res = all_results[all_results[DIAGEOUSConst.MANUFACTURER] == manufacturer][DIAGEOUSConst.PASSED].sum()
        ...
        self.common.write_to_db_result(
            ...
            identifier_result=total_dict, identifier_parent=self.common.get_dictionary(name=DIAGEOUSConst.TOTAL))
    ...
    def calculate_display_share_of_sku(self, product_fk, relevant_products, manufacturer_kpi_fk):
        ...
        """
        sku_kpi_fk = self.common.get_kpi_fk_by_kpi_name(DIAGEOUSConst.DB_OFF_NAMES[DIAGEOUSConst.DISPLAY_SHARE][DIAGEOUSConst.SKU])
        ...
        product_result = {DIAGEOUSConst.PRODUCT_FK: product_fk, DIAGEOUSConst.PASSED: sum_scenes_passed,
                          DIAGEOUSConst.MANUFACTURER: manufacturer}
        return product_result
    ...
    def calculate_total_shelf_facings(self, scene_types, kpi_name, weight):
        ...
        """
        relevant_scenes = self.get_relevant_scenes(scene_types)
        relevant_competitions = self.templates[DIAGEOUSConst.SHELF_FACING_SHEET]
        if self.state in relevant_competitions[DIAGEOUSConst.STATE].unique().tolist():
            relevant_competitions = relevant_competitions[relevant_competitions[DIAGEOUSConst.STATE] == self.state]
        else:
            relevant_competitions = relevant_competitions[relevant_competitions[DIAGEOUSConst.STATE] == DIAGEOUSConst.OTHER]
        all_results = pd.DataFrame(columns=DIAGEOUSConst.COLUMNS_FOR_PRODUCT)
    ...
    def calculate_shelf_facings_of_competition(self, competition, relevant_scenes, index):
        ...
        """
        kpi_fk = self.common.get_kpi_fk_by_kpi_name(DIAGEOUSConst.DB_OFF_NAMES[DIAGEOUSConst.SHELF_FACINGS][DIAGEOUSConst.COMPETITION])
        our_eans = competition[DIAGEOUSConst.OUR_EAN_CODE].split(', ')
        ...
        if self.does_exist(competition[DIAGEOUSConst.COMP_EAN_CODE]):
            comp_eans = competition[DIAGEOUSConst.COMP_EAN_CODE].split(', ')
            ...
            bench_value = competition[DIAGEOUSConst.BENCH_VALUE]
            # bench_value = float(bench_value.replace("%", ""))/100
            target = comp_facings * bench_value
        else:
            target = competition[DIAGEOUSConst.BENCH_VALUE]
        comparison = 1 if (our_facings >= target and our_facings > 0) else 0
        # result = self.get_score(our_facings, all_facings)
        brand, sub_brand = self.get_product_details(product_fk)
        total_kpi_fk = self.common.get_kpi_fk_by_kpi_name(DIAGEOUSConst.DB_OFF_NAMES[DIAGEOUSConst.SHELF_FACINGS][DIAGEOUSConst.TOTAL])
        self.common.write_to_db_result(
            fk=kpi_fk, numerator_id=product_fk,
        ...
        product_result = {DIAGEOUSConst.PRODUCT_FK: product_fk, DIAGEOUSConst.PASSED: comparison,
                          DIAGEOUSConst.BRAND: brand, DIAGEOUSConst.SUB_BRAND: sub_brand}
        return product_result
    ...
    def calculate_shelf_facings_of_sku(self, product_fks, relevant_scenes, parent_identifier):
        ...
        """
        kpi_fk = self.common.get_kpi_fk_by_kpi_name(DIAGEOUSConst.DB_OFF_NAMES[DIAGEOUSConst.SHELF_FACINGS][DIAGEOUSConst.SKU])
        amount_of_facings = 0
    ...
    def convert_groups_from_template(self):
        ...
        """
        shelf_groups = self.templates[DIAGEOUSConst.SHELF_GROUPS_SHEET]
        shelves_groups = {}
        for i, group in shelf_groups.iterrows():
            shelves_groups[group[DIAGEOUSConst.NUMBER_GROUP]] = group[DIAGEOUSConst.SHELF_GROUP].split(', ')
    ...
    def calculate_total_shelf_placement(self, scene_types, kpi_name, weight):
        ...
        """
        relevant_scenes = self.get_relevant_scenes(scene_types)
        all_products_table = self.templates[DIAGEOUSConst.SHELF_PLACMENTS_SHEET]
        all_results = pd.DataFrame(columns=DIAGEOUSConst.COLUMNS_FOR_PRODUCT)
    ...
    def calculate_shelf_placement_of_sku(self, product_line, relevant_scenes):
        ...
        :return:
        """
        kpi_fk = self.common.get_kpi_fk_by_kpi_name(DIAGEOUSConst.DB_OFF_NAMES[DIAGEOUSConst.SHELF_PLACEMENT][DIAGEOUSConst.SKU])
        total_kpi_fk = self.common.get_kpi_fk_by_kpi_name(DIAGEOUSConst.DB_OFF_NAMES[DIAGEOUSConst.SHELF_PLACEMENT][DIAGEOUSConst.TOTAL])
        product_fk = self.all_products[self.all_products['product_ean_code'] == product_line[
            DIAGEOUSConst.PRODUCT_EAN_CODE]]['product_fk']
        if product_fk.empty:
            Log.warning("Product_ean '{}' does not exist".format(product_line[DIAGEOUSConst.PRODUCT_EAN_CODE]))
            return None
        product_fk = product_fk.iloc[0]
        min_shelf_loc = product_line[DIAGEOUSConst.MIN_SHELF_LOCATION]
        shelf_groups = self.converted_groups[min_shelf_loc]
        min_max_shleves = self.templates[DIAGEOUSConst.MINIMUM_SHELF_SHEET]
        calculate_all = False
        if "ALL" in shelf_groups:
            relevant_groups = min_max_shleves
            calculate_all = True
        else:
            relevant_groups = min_max_shleves[min_max_shleves[DIAGEOUSConst.SHELF_NAME].isin(shelf_groups)]
        ...
        result = 100 if eye_level_product_facings > (all_facings * DIAGEOUSConst.PERCENT_FOR_EYE_LEVEL) else 0
        brand, sub_brand = self.get_product_details(product_fk)
        self.common.write_to_db_result(
            fk=kpi_fk, numerator_id=product_fk, result=result,
            identifier_parent=self.common.get_dictionary(kpi_fk=total_kpi_fk))
        product_result = {DIAGEOUSConst.PRODUCT_FK: product_fk, DIAGEOUSConst.PASSED: result / 100,
                          DIAGEOUSConst.BRAND: brand, DIAGEOUSConst.SUB_BRAND: sub_brand}
        return product_result
    ...
    def calculate_specific_product_eye_level(self, match_product_line, relevant_groups, calculate_all):
        ...
        relevant_groups_for_scene = relevant_groups[
            (relevant_groups[DIAGEOUSConst.NUM_SHLEVES_MIN] <= self.scenes_with_shelves[scene]) &
            (relevant_groups[DIAGEOUSConst.NUM_SHLEVES_MAX] >= self.scenes_with_shelves[scene])]
        if relevant_groups_for_scene.empty:
            return 0
        relevant_shelves = relevant_groups_for_scene[DIAGEOUSConst.SHELVES_FROM_BOTTOM].tolist()
        if calculate_all:
    ...
    def calculate_total_msrp(self, scene_types, kpi_name, weight):
        ...
        """
        relevant_scenes = self.get_relevant_scenes(scene_types)
        all_products_table = self.templates[DIAGEOUSConst.PRICING_SHEET]
        if self.state in all_products_table[DIAGEOUSConst.STATE].unique().tolist():
            all_products_table = all_products_table[all_products_table[DIAGEOUSConst.STATE] == self.state]
        else:
            all_products_table = all_products_table[all_products_table[DIAGEOUSConst.STATE] == DIAGEOUSConst.OTHER]
        all_competes = pd.DataFrame(columns=DIAGEOUSConst.COLUMNS_FOR_PRODUCT)
    ...
    def calculate_msrp_of_competition(self, competition, relevant_scenes, index):
        ...
        :return: 1/0
        """
        kpi_fk = self.common.get_kpi_fk_by_kpi_name(DIAGEOUSConst.DB_OFF_NAMES[DIAGEOUSConst.MSRP][DIAGEOUSConst.COMPETITION])
        total_kpi_fk = self.common.get_kpi_fk_by_kpi_name(DIAGEOUSConst.DB_OFF_NAMES[DIAGEOUSConst.MSRP][DIAGEOUSConst.TOTAL])
        our_ean = competition[DIAGEOUSConst.OUR_EAN_CODE]
        our_line = self.all_products[self.all_products['product_ean_code'] == our_ean]
        min_relative, max_relative = competition[DIAGEOUSConst.MIN_MSRP_RELATIVE], competition[DIAGEOUSConst.MAX_MSRP_RELATIVE]
        min_absolute, max_absolute = competition[DIAGEOUSConst.MIN_MSRP_ABSOLUTE], competition[DIAGEOUSConst.MAX_MSRP_ABSOLUTE]
        if our_line.empty:
        ...
        if our_price is None:
            return None
        is_competitor = (self.does_exist(competition[DIAGEOUSConst.COMP_EAN_CODE]) and
        ...
        if is_competitor:
            comp_ean = competition[DIAGEOUSConst.COMP_EAN_CODE]
            ...
            if comp_price is None:
                return None
            range_price = (round(comp_price + competition[DIAGEOUSConst.MIN_MSRP_RELATIVE], 2),
                           round(comp_price + competition[DIAGEOUSConst.MAX_MSRP_RELATIVE], 2))
        elif is_absolute:
            range_price = (competition[DIAGEOUSConst.MIN_MSRP_ABSOLUTE], competition[DIAGEOUSConst.MAX_MSRP_ABSOLUTE])
        else:
        ...
        product_result = {DIAGEOUSConst.PRODUCT_FK: product_fk, DIAGEOUSConst.PASSED: (result == 0) * 1,
                          DIAGEOUSConst.BRAND: brand, DIAGEOUSConst.SUB_BRAND: sub_brand}
        return product_result
    ...
    def calculate_sku_price(self, product_fk, scenes, parent_dict):
        ...
        :return: price
        """
        kpi_fk = self.common.get_kpi_fk_by_kpi_name(DIAGEOUSConst.DB_OFF_NAMES[DIAGEOUSConst.MSRP][DIAGEOUSConst.SKU])
    ...
    def calculate_passed_display(self, product_fk, relevant_products):
        ...
        """
        template = self.templates[DIAGEOUSConst.DISPLAY_TARGET_SHEET]
        ...
        for scene in relevant_products['scene_fk'].unique().tolist():
            ...
            minimum_products = template[template[DIAGEOUSConst.SCENE_TYPE] == scene_type]
            if minimum_products.empty:
                minimum_products = template[template[DIAGEOUSConst.SCENE_TYPE] == DIAGEOUSConst.OTHER]
            minimum_products = minimum_products[DIAGEOUSConst.MIN_FACINGS].iloc[0]
    ...
    def get_templates(self):
        ...
        """
        if self.on_premise:
            for sheet in DIAGEOUSConst.ON_SHEETS:
                self.templates[sheet] = pd.read_excel(ON_TEMPLATE_PATH, sheetname=sheet, skiprows=2,
                                                      keep_default_na=False)
        else:
            for sheet in DIAGEOUSConst.OFF_SHEETS:
                if sheet in ([DIAGEOUSConst.SHELF_FACING_SHEET, DIAGEOUSConst.PRICING_SHEET]):
                    converters = {DIAGEOUSConst.OUR_EAN_CODE: lambda x: str(x).replace(".0", ""),
                                  DIAGEOUSConst.COMP_EAN_CODE: lambda x: str(x).replace(".0", "")}
                ...
                elif sheet == DIAGEOUSConst.SHELF_PLACMENTS_SHEET:
                    converters = {DIAGEOUSConst.PRODUCT_EAN_CODE: lambda x: str(x).replace(".0", "")}
                    self.templates[sheet] = pd.read_excel(OFF_TEMPLATE_PATH, sheetname=sheet, skiprows=2,
                                                          converters=converters, keep_default_na=False)
                else:
                    self.templates[sheet] = pd.read_excel(OFF_TEMPLATE_PATH, sheetname=sheet, skiprows=2,
    ...
    def insert_all_levels_to_db(self, all_results, kpi_name, weight, with_standard_type=False):
        ...
        :return:
        """
        total_kpi_fk = self.common.get_kpi_fk_by_kpi_name(DIAGEOUSConst.DB_OFF_NAMES[kpi_name][DIAGEOUSConst.TOTAL])
        total_identifier = self.common.get_dictionary(kpi_fk=total_kpi_fk)
        for brand in all_results[DIAGEOUSConst.BRAND].unique().tolist():
            brand_results = all_results[all_results[DIAGEOUSConst.BRAND] == brand]
            self.insert_brand_and_subs_to_db(brand_results, kpi_name, brand, total_identifier)
        all_passed_results = all_results[DIAGEOUSConst.PASSED]
        total_result = self.insert_totals_to_db(all_passed_results, kpi_name, DIAGEOUSConst.TOTAL, weight, total_identifier)
        segment_result, national_result = 0, 0
        if with_standard_type:
            national_results = all_results[all_results[DIAGEOUSConst.STANDARD_TYPE] == DIAGEOUSConst.NATIONAL][DIAGEOUSConst.PASSED]
            national_result = self.insert_totals_to_db(national_results, kpi_name, DIAGEOUSConst.NATIONAL, weight)
            segment_results = all_results[all_results[DIAGEOUSConst.STANDARD_TYPE] == DIAGEOUSConst.SEGMENT][DIAGEOUSConst.PASSED]
            segment_result = self.insert_totals_to_db(segment_results, kpi_name, DIAGEOUSConst.SEGMENT, weight)
        return total_result, segment_result, national_result
    ...
    def insert_brand_and_subs_to_db(self, brand_results, kpi_name, brand, total_identifier):
        ...
        :return:
        """
        brand_kpi_fk = self.common.get_kpi_fk_by_kpi_name(DIAGEOUSConst.DB_OFF_NAMES[kpi_name][DIAGEOUSConst.BRAND])
        brand_dict = self.common.get_dictionary(kpi_fk=brand_kpi_fk, brand_fk=brand)
        for sub_brand in brand_results[brand_results[DIAGEOUSConst.BRAND] == brand][DIAGEOUSConst.SUB_BRAND].unique().tolist():
            sub_brand_results = brand_results[(brand_results[DIAGEOUSConst.BRAND] == brand) &
                                              (brand_results[DIAGEOUSConst.SUB_BRAND] == sub_brand)]
            self.insert_sub_brands_to_db(sub_brand_results, kpi_name, brand, sub_brand, brand_dict)
        results = brand_results[DIAGEOUSConst.PASSED]
    ...
    def insert_sub_brands_to_db(self, sub_brand_results, kpi_name, brand, sub_brand, brand_identifier):
        ...
        :return:
        """
        sub_brand_kpi_fk = self.common.get_kpi_fk_by_kpi_name(DIAGEOUSConst.DB_OFF_NAMES[kpi_name][DIAGEOUSConst.SUB_BRAND])
        sub_brand_dict = self.common.get_dictionary(kpi_fk=sub_brand_kpi_fk, brand_fk=brand, sub_brand_fk=sub_brand)
        results = sub_brand_results[DIAGEOUSConst.PASSED]
        num_res, den_res = results.sum(), results.count()
        self.common.write_to_db_result(
    ...
    def insert_totals_to_db(self, all_passed_results, kpi_name, total_kind, weight, identifier_result=None):
        ...
        :return:
        """
        kpi_fk = self.common.get_kpi_fk_by_kpi_name(DIAGEOUSConst.DB_OFF_NAMES[kpi_name][total_kind])
        num_result, den_result = all_passed_results.sum(), all_passed_results.count()
        result = self.get_score(num_result, den_result)
        score = result * weight if kpi_name != DIAGEOUSConst.MSRP else 0
        self.common.write_to_db_result(
