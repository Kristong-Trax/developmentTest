...
class LiveAssortmentCalculation:
    ...
    def get_lvl3_relevant_ass(self):
        ...
        if self.store_assortment.empty:
            return pd.DataFrame()
        assortment_result = self.store_assortment.loc[self.store_assortment['start_date'] <= self.current_date][self.LVL3_HEADERS]
        assortment_result['in_store'] = 0
    ...
    def calculate_lvl2_assortment(self, lvl3_assortment):
        ...
        (total\ target).
        """
        lvl2_res = lvl3_assortment.groupby([self.ASSORTMENT_FK, self.ASSORTMENT_GROUP_FK])[self.LVL2_HEADERS].agg(
            passes=('in_store', 'sum'), total=('in_store', 'count')).reset_index()
        return lvl2_res
