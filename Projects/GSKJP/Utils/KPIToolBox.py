...
class GSKJPToolBox:
    ...
    def __init__(self, data_provider, output):
        ...
        self.set_up_data = {(Consts.PLN_BLOCK, GlobalConsts.KPI_TYPE_COLUMN): GlobalConsts.NO_INFO,
                            (Consts.POSITION_SCORE, GlobalConsts.KPI_TYPE_COLUMN):
                                GlobalConsts.NO_INFO, (Consts.ECAPS_FILTER_IDENT, GlobalConsts.KPI_TYPE_COLUMN):
                                GlobalConsts.NO_INFO, (Consts.PLN_MSL, GlobalConsts.KPI_TYPE_COLUMN):
                                GlobalConsts.NO_INFO, ("GSK_PLN_LSOS_SCORE",
                                                GlobalConsts.KPI_TYPE_COLUMN): GlobalConsts.NO_INFO}
    ...
    def brand_blocking(self, brand, policy):
        ...
        """
        templates = self.set_up_data[(GlobalConsts.SCENE_TYPE, Consts.PLN_BLOCK)]
        template_name = {
    ...
    def pln_ecaps_score(self, brand, assortment):
        ...
        for result in brand_results.itertuples():
            if (math.isnan(result.in_store)) | (result.kpi_fk_lvl3 != ecaps_assortment_fk):
                score = self.gsk_generator.tool_box.result_value_pk(GlobalConsts.EXTRA)
                result_num = 1
            else:
                score = self.gsk_generator.tool_box.result_value_pk(GlobalConsts.OOS) if result.in_store == 0 else \
                    self.gsk_generator.tool_box.result_value_pk(GlobalConsts.DISTRIBUTED)
    ...
    def gsk_compliance(self):
        ...
        assortment_msl = self.msl_assortment(GlobalConsts.DISTRIBUTION, Consts.PLN_MSL)
