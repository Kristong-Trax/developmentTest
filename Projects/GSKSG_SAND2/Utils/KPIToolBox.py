...
class GSKSGToolBox:
    ...
    def planogram(self, category_fk, assortment, category_targets, identifier_parent):
        ...
        planogram_result = planogram_score / float(planogram_weight) if planogram_weight else 0
    ...
    def _save_sequence_main_level_to_db(self, kpi_fk, planogram_identifier, cat_fk, sequence_score, total_weight):
        """
        This method saves the top sequence level to DB.
        """
        result = round((sequence_score / float(total_weight)), 2) if total_weight else 0
        score = result * total_weight
        self.common.write_to_db_result(fk=kpi_fk, numerator_id=cat_fk, numerator_result=sequence_score,
                                       result=result, denominator_id=self.store_id, denominator_result=total_weight,
                                       score=score, weight=total_weight, target=total_weight, should_enter=True,
    ...
    def _save_sequence_results_to_db(self, kpi_fk, parent_kpi_fk, sequence_params, sequence_results):
        ...
        self.common.write_to_db_result(fk=kpi_fk, numerator_id=category_fk, numerator_result=num_of_sequences,
                                       result=result_value, denominator_id=self.store_id, denominator_result=None,
