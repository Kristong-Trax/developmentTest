...
class INBEVLU_SANDINBEVBEToolBox:
    LEVEL1 = 1
    LEVEL2 = 2
    LEVEL3 = 3
    ...
    def __init__(self, data_provider, output):
        ...
        self.tools = INBEVLU_SANDINBEVBEINBEVToolBox(self.data_provider, output,
    ...
    def get_business_unit(self):
        ...
        """
        query = INBEVLU_SANDINBEVBEQueries.get_business_unit_data(self.store_info['store_fk'].values[0])
    ...
    def get_kpi_static_data(self):
        ...
        """
        query = INBEVLU_SANDINBEVBEQueries.get_all_kpi_data()
    ...
    def get_match_display(self):
        ...
        """
        query = INBEVLU_SANDINBEVBEQueries.get_match_display(self.session_uid)
        match_display = pd.read_sql_query(query, self.rds_conn.db)
        return match_display

    def get_osa_table(self):
        query = INBEVLU_SANDINBEVBEQueries.get_osa_table(self.store_id)
        osa_table = pd.read_sql_query(query, self.rds_conn.db)
        return osa_table

    def get_oos_messages(self):
        query = INBEVLU_SANDINBEVBEQueries.get_oos_messages(self.session_uid)
        oos_messages = pd.read_sql_query(query, self.rds_conn.db)
        return oos_messages

    def get_store_number_1(self):
        query = INBEVLU_SANDINBEVBEQueries.get_store_number_1(self.store_id)
    ...
    def get_eye_level_shelves(self, shelves_num):
        jg = INBEVLU_SANDINBEVBEJsonGenerator('inbevbe')
    ...
    def remove_product_from_store_assortment(self, products_list):
        for product in products_list:
            custom_osa_query = INBEVLU_SANDINBEVBEQueries.get_delete_osa_records_query(product, self.store_id, datetime.utcnow())
    ...
    def get_store_attribute_8(self, store_fk):
        query = INBEVLU_SANDINBEVBEQueries.get_store_attribute_8(store_fk)
        final_df = pd.read_sql_query(query, self.rds_conn.db)
        value = final_df.values[0][0]
        return value

    def get_all_products(self):
        query = INBEVLU_SANDINBEVBEQueries.get_att3_att4_for_products()
    ...
    def calculate_block_together_sets(self, set_name):
        ...
        for group in product_groups:
            ...
            if not relevant_df.empty:
                ...
                product_atomic_names = relevant_df['SKU Name (Description)'].unique().tolist()
    ...
    def get_product_att4(self, product_fk):
        query = INBEVLU_SANDINBEVBEQueries.get_product_att4(product_fk)
        att4 = pd.read_sql_query(query, self.rds_conn.db)
        return att4.values[0][0]

    def get_rect_values(self):
        query = INBEVLU_SANDINBEVBEQueries.get_rect_values_query(self.session_uid)
    ...
    def commit_results_data(self):
        ...
        delete_queries = INBEVLU_SANDINBEVBEQueries.get_delete_session_results_query(self.session_uid)
        pservice_tables_delete_query = INBEVLU_SANDINBEVBEQueries.get_delete_custom_scif_query(self.session_fk)
