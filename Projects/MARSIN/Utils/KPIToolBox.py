...
from Projects.MARSIN.Utils.Fetcher import MARSINQueries
from Projects.MARSIN.Utils.GeneralToolBox import MARSINGENERALToolBox
from Projects.MARSIN.Utils.ParseComplexTemplates import parse_template
...
class MARSINToolBox(MARSINTemplateConsts, MARSINKPIConsts):
    ...
    def main_calculation(self):
        ...
        for p in xrange(len(self.kpi_data)):
            ...
            if kpi_name_context.empty:
                Log.warning(
                    "KPI '{}' & KPI GROUP '{}' does not exist in kpi_result_value".format(
            ...
            if self.validate_kpi_run(kpi_group, kpi_name):
                ...
                if kpi_score is not None:
                    ...
                    identifier_result = self.common.get_dictionary(
                        group_name=params[MARSINTemplateConsts.KPI_GROUP],
                        brand_sub=params[MARSINTemplateConsts.KPI_NAME])
        ...
        for group_name in group_scores:
            ...
            identifier_parent = self.common.get_dictionary(kpi_name=MARSINKPIConsts.PICOS)
    ...
    def save_picos_hierarchy(self, pillar_scores):
        ...
        identifier_result = self.common.get_dictionary(kpi_name=MARSINKPIConsts.PICOS)
        identifier_parent = self.common.get_dictionary(kpi_name='PICOS')
        kpi_level_2_fk = self.common.get_kpi_fk_by_kpi_type(MARSINKPIConsts.PICOS)
        self.common.write_to_db_result(fk=kpi_level_2_fk,
                                       numerator_id=self.manufacturer_fk,
                                       denominator_id=self.store_id,
                                       numerator_result=actual_points,
    ...
    def validate_kpi_run(self, set_name, kpi_name):
        ...
        if not data.empty:
            try:
                ...
                Log.warning("Store Type:{} not found in KPI Template for Session_uid{}".format(self.store_type,
                                                                                               self.session_uid))
        return kpi_run
    ...
    def calculate_sequence_within_block(self, params, atomics, kpi_name_context):
        ...
        identifier_parent = self.common.get_dictionary(group_name=params[MARSINTemplateConsts.KPI_GROUP],
                                                       brand_sub=params[MARSINTemplateConsts.KPI_NAME])
        if kpi_data.empty:
            return None
    ...
    def calculate_availability_for_shelves(self, params, atomic_fk, kpi_name_context):
        ...
        # write results to new db
        # identifier_result = self.common.get_dictionary(group_name=params[MARSINTemplateConsts.KPI_GROUP],
        #                                                brand_sub=params[MARSINTemplateConsts.KPI_NAME])
        identifier_parent = self.common.get_dictionary(group_name=params[MARSINTemplateConsts.KPI_GROUP],
                                                       brand_sub=params[MARSINTemplateConsts.KPI_NAME])
