...
class ToolBox:
    ...
    def calculate_main_kpi(self, main_line):
        ...
        scene_types = self.read_cell_from_line(main_line, 'Scene Type')
        general_filters = {}
        ...
        if (self.super_cat in ['YOGURT']) and \
                (kpi_type in [
                    Const.COUNT_SHELVES,
                    Const.COUNT,
    ...
    def calculate_base_measure(self, kpi_name, kpi_line, relevant_scif, general_filters):
        ...
        if total < unit:
            result = 'Less than or Equal to {} Feet'.format(unit)
        elif total > max_res:
            result = 'Greater than {} Feet'.format(max_res)
        elif total % unit == 0:
            result = '{} Feet > and <= {} Feet'.format(ft - unit, ft)
        else:
            result = '{} Feet > and <= {} Feet'.format(ft, ft + unit)
    ...
    def calculate_max_block_adjacency_subcat(self, kpi_name, kpi_line, relevant_scif, general_filters):
        ...
        if adj == 0:
            ...
            pre_result_list = []
            for segment in adj:
                result_substring = segment.replace('TTL VS ', "").split(' ')[0]
                potential_results = self.get_results_value(kpi_line)
    ...
    def calculate_max_block_adj_base_adjacency_compare(self, kpi_name, kpi_line, relevant_scif, general_filters,
                                                       comp_filter={},
        ...
        for k, v in d.items():
            ...
            if self.read_cell_from_line(kpi_line, 'Max Block')[0] == 'Y':
                v['df'] = pd.DataFrame(v['df'].iloc[0, :]).T
            if k in thresh:
                if v['df']['total_facings'].iloc[0] / float(self.filter_df(self.full_mpis, filters).shape[0]) <= thresh[
                    k]:
                    return 0
            v['items'] = sum([list(n['match_fk']) for j, row in v['df'].iterrows()
                              for i, n in row['cluster'].nodes(data=True)
    ...
    @empty_scif_decorator
    def calculate_blocking_presence(self, kpi_name, kpi_line, relevant_scif, general_filters):
        _, orientation, mpis_dict, blocks, blocking_results, = self.base_block(kpi_name, kpi_line, relevant_scif,
                                                                               general_filters)
        potential_results = self.get_results_value(kpi_line)
        ...
        if not blocks.empty:
            ...
            nb_result = orientation  # Should be 'Not blocked' for varaible value, if blocks.empty is true
            match = [res for res in potential_results if orientation in res or nb_result.lower() in res]
            if len(match) > 0:
                result = match[0]
    ...
    def calculate_anchor(self, kpi_name, kpi_line, relevant_scif, general_filters):
        ...
        for scene in scenes:
            ...
            for count in counts.index:
                result = ' '.join([sub for sub in count.split(' ')])
    ...
    def calculate_count_shelves(self, kpi_name, kpi_line, relevant_scif, general_filters):
        ...
        result = self.semi_numerical_results(mode_of_shelves[0], potential_results, extract_result_int=True,
                                             form='{} Shelves')
    ...
    def base_count(self, kpi_name, kpi_line, relevant_scif, general_filters, min=0):
        filters = self.get_kpi_line_filters(kpi_line)
        filters.update(general_filters)
        count_col = self.read_cell_from_line(kpi_line, 'Counted Attribute')
        scif = self.filter_df(relevant_scif, filters)
    ...
    def write_to_db(self, kpi_name, score=0, result=None, target=None, numerator_result=0,
        ...
        :param threshold: int
        """
        # print ('Saving for kpi: {}'.format(kpi_name)) # debugging saving
            ...
            if not failed:
                ...
                if type(result) == list:
                    for specific_result in result:
                        result = self.result_values_dict[specific_result]
