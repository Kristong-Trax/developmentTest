...
from Projects.PNGRO.Utils.PositionGraph import PNGRO_PRODPositionGraphs

__author__ = 'Israel'


class PNGRO_PRODGENERALToolBox:
    ...
    def position_graphs(self):
        if not hasattr(self, '_position_graphs'):
            self._position_graphs = PNGRO_PRODPositionGraphs(self.data_provider, rds_conn=self.rds_conn)
    ...
    def calculate_availability(self, **filters):
        ...
        if set(filters.keys()).difference(self.match_product_in_scene.keys()):
            filtered_df = self.scif[self.get_filter_condition(self.scif, **filters)]
        else:
            filtered_df = self.match_product_in_scene[self.get_filter_condition(self.match_product_in_scene, **filters)]
    ...
    def calculate_block_together(self, allowed_products_filters=None, include_empty=EXCLUDE_EMPTY,
                                 minimum_block_ratio=1, result_by_scene=False,
                                 block_of_blocks=False, block_products1=None, block_products2=None, **filters):
        """
