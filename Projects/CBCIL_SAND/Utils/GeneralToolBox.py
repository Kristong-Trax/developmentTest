...
class CBCILCBCIL_GENERALToolBox:
    ...
    def match_product_in_scene(self):
        if not hasattr(self, '_match_product_in_scene'):
            ...
            # self._match_product_in_scene = self._match_product_in_scene.merge(self.scif[['product_fk', 'brand_name', 'size']], on='product_fk')
                ...
                if block_of_blocks:
                    ...
                    if cluster_ratio >= minimum_block_ratio:
                        if result_by_scene:
                            ...
                            if vertical:
                                return {'block': True, 'shelves': set(scene_graph.vs['shelf_number'])}
    ...
    def separate_location_filters_from_product_filters(self, **filters):
        ...
        """
        relevant_scenes = self.match_product_in_scene[self.get_filter_condition(self.match_product_in_scene, **filters)]['scene_id'].unique()
        location_filters = {}
        for field in filters.keys():
