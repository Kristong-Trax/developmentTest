...
from Projects.CCUS_SAND.GOLD_PEAK_BLOCK.Utils.PositionGraph import CCUS_SANDGOLD_PEAK_BLOCKPositionGraphs

__author__ = 'Ortal'


class CCUS_SANDGOLD_PEAK_BLOCKGeneralCCUS_SANDToolBox:
    ...
    def get_atts(self):
        ...
        query = CCUS_SANDGOLD_PEAK_BLOCKQueries.get_product_atts()
    ...
    def position_graphs(self):
        if not hasattr(self, '_position_graphs'):
            self._position_graphs = CCUS_SANDGOLD_PEAK_BLOCKPositionGraphs(self.data_provider, rds_conn=self.rds_conn)
        ...
        for scene in relevant_scenes:
            ...
            for cluster in clusters:
                ...
                if cluster_ratio > minimum_block_ratio:
                    ...
                    result_v = self.calculate_relative_in_block_per_graph(block_graph, direction_data)
                    direction_data = {'left': (2, 2), 'right': (2, 2)}
                    result_h = self.calculate_relative_in_block_per_graph(block_graph, direction_data)
