...
from Projects.CCUS_SAND.GOLD_PEAK_BLOCK.Utils.Fetcher import CCUS_SANDGOLD_PEAK_BLOCKQueries
from Projects.CCUS_SAND.GOLD_PEAK_BLOCK.Utils.GeneralToolBox import CCUS_SANDGOLD_PEAK_BLOCKGeneralCCUS_SANDToolBox
from Projects.CCUS_SAND.GOLD_PEAK_BLOCK.Utils.ParseTemplates import parse_template
...
class GOLD_PEAK_BLOCKCCUS_SANDToolBox:
    LEVEL1 = 1
    LEVEL2 = 2
    LEVEL3 = 3
    ...
    def __init__(self, data_provider, output):
        ...
        self.tools = CCUS_SANDGOLD_PEAK_BLOCKGeneralCCUS_SANDToolBox(self.data_provider, self.output, rds_conn=self.rds_conn)
    ...
    def get_kpi_static_data(self):
        ...
        """
        query = CCUS_SANDGOLD_PEAK_BLOCKQueries.get_all_kpi_data()
    ...
    def commit_results_data(self, kpi_set_fk=None):
        ...
        if kpi_set_fk is not None:
            query = CCUS_SANDGOLD_PEAK_BLOCKQueries.get_atomic_pk_to_delete(self.session_uid, kpi_set_fk)
            kpi_atomic_data = pd.read_sql_query(query, self.rds_conn.db)
            atomic_pks = tuple(kpi_atomic_data['pk'].tolist())
        cur = self.rds_conn.db.cursor()
        if atomic_pks:
            delete_queries = CCUS_SANDGOLD_PEAK_BLOCKQueries.get_delete_session_results_query(self.session_uid, kpi_set_fk, atomic_pks)
