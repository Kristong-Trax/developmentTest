...
class CCUS_SANDNewTemplate:

    PRODUCT_FIELDS_TO_VALIDATE = {'product_ean_code': [CCUS_SANDConsts.PRODUCT_EAN_CODE, CCUS_SANDConsts.PRODUCT_EAN_CODE2,
                                                       CCUS_SANDConsts.TESTED, CCUS_SANDConsts.ANCHOR],
                                  'brand_name': [CCUS_SANDConsts.BRAND_NAME],
                                  'sub_brand_name': [CCUS_SANDConsts.SUB_BRAND_NAME]}
    TEMPLATE_FIELDS_TO_VALIDATE = {'template_name': [CCUS_SANDConsts.LOCATION]}
    REQUIRED_FIELDS_FOR_SET = {'Availability': [(CCUS_SANDConsts.PRODUCT_EAN_CODE, CCUS_SANDConsts.PRODUCT_EAN_CODE2)],
                               'Relative Position': [CCUS_SANDConsts.CHANNEL, CCUS_SANDConsts.LOCATION, CCUS_SANDConsts.TESTED, CCUS_SANDConsts.ANCHOR,
                                                     CCUS_SANDConsts.TOP_DISTANCE, CCUS_SANDConsts.BOTTOM_DISTANCE, CCUS_SANDConsts.LEFT_DISTANCE,
                                                     CCUS_SANDConsts.RIGHT_DISTANCE],
                               'Brand Blocking': [CCUS_SANDConsts.BRAND_NAME],
                               'Brand Pouring': [CCUS_SANDConsts.BRAND_NAME]}
    SEPARATOR = ','
    ...
    def __init__(self, project):
        ...
        self.kpi_name_field = CCUS_SANDConsts.KPI_NAME
    ...
    def handle_updated_template(self, set_name, template_path, ignore_missings=False, replace_existing=False):
        ...
        data = CCUS_SANDConsts.get_json_data(template_path)
        if data is None:
            return False
    ...
    def get_latest_templates(self):
        ...
        """
        latest_templates = {}
        latest_date = CCUS_SANDConsts.get_latest_directory_date_from_cloud(self.templates_path.format(''), self.amz_conn)
        if latest_date:
    ...
    def add_dummy_kpi(self, set_name):
        ...
        """
        params = {CCUS_SANDConsts.PRODUCT_NAME: set_name,
                  CCUS_SANDConsts.KPI_NAME: set_name}
    ...
    def add_new_kpi_to_static_tables(self, set_fk, new_kpi_list):
        ...
        for kpi in new_kpi_list:
            if kpi:
                ...
                # product_ean_code = str(kpi.get(CCUS_SANDConsts.PRODUCT_EAN_CODE, kpi.get(CCUS_SANDConsts.PRODUCT_EAN_CODE2, '')))
                level2_query = """
    ...
    def validate_template(self, set_name, data, ignore_missings=False):
        ...
        if self.kpi_name_field not in fields_of_data:
            ...
            return False
        if set_name in CCUS_SANDConsts.AVAILABILITY_SETS:
    ...
    def add_kpi_sets_to_static(self):
        ...
        """
        cur = self.rds_conn.db.cursor()
        for set_name in CCUS_SANDConsts.KPI_SETS:
...
#     for project_name in ['ccus_sand']:
#         LoggerInitializer.init('{} New Template'.format(project_name))
#         template = CCUS_SANDNewTemplate(project_name)
