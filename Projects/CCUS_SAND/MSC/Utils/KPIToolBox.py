...
from Projects.CCUS_SAND.MSC.Utils.GeneralToolBox import MSCGENERALToolBox
from Projects.CCUS_SAND.MSC.Utils.ParseTemplates import parse_template
...
class MSCToolBox:
    ...
    def __init__(self, data_provider, output):
        ...
        self.scif_filters = {'In Scene Type': 'template_name', 'Template Group': 'template_group', 'Excluded Template Group':'template_group', 'Manufacturer':'manufacturer_name',
                             'Manufacturer Exclude': 'manufacturer_name', 'Trademark (att2)': 'att2', 'Excluded Trademark (att2)':'att2', 'Sub Category': 'sub_category',
        ...
        self.row_filters =  ['In Scene Type', 'Template Group', 'Excluded Template Group', 'Manufacturer', 'Manufacturer Exclude', 'Trademark (att2)', 'Excluded Trademark (att2)', 'Sub Category',
    ...
    def count_unique(self, kpi_name, row, assortment_entity):
        ...
        if row['Saved'] == 'Y':
            atomic_fk = self.kpi_static_data[(self.kpi_static_data['atomic_kpi_name'] == atomic_name) &
                                         (self.kpi_static_data['kpi_name'] == kpi_name)]['atomic_kpi_fk'].values[0]
            self.write_to_db_result(atomic_fk, score, level=self.LEVEL3, result=result)
    ...
    def check_survey_answer(self, kpi_name, row):
        ...
        Questions = row['Question fk'].split(",")
        for question in Questions:
            survey_answer = self.tools.get_survey_answer(('question_fk', int(question)))
    ...
    def commit_results_data(self,kpi_set_fk=None):
        ...
        if kpi_set_fk is not None:
            ...
            query = MSCQueries.get_kpi_pk_to_delete(self.session_uid, kpi_set_fk)
