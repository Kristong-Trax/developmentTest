...
from Projects.CCUS_SAND.Holiday.Utils.Fetcher import CCUS_SANDHolidayQueries
from Projects.CCUS_SAND.Holiday.Utils.GeneralToolBox import CCUS_SANDHolidayGENERALCCUS_SANDToolBox
from Projects.CCUS_SAND.Holiday.Utils.ParseTemplates import parse_template
...
class CCUS_SANDHOLIDAYCCUS_SANDToolBox:
    LEVEL1 = 1
    LEVEL2 = 2
    LEVEL3 = 3
    ...
    def __init__(self, data_provider, output):
        ...
        self.tools = CCUS_SANDHolidayGENERALCCUS_SANDToolBox(self.data_provider, self.output, rds_conn=self.rds_conn)
    ...
    def get_kpi_static_data(self):
        ...
        """
        query = CCUS_SANDHolidayQueries.get_all_kpi_data()
    ...
    def create_attributes_dict(self, score, level, result=None, name=None):
        ...
        """
        if level == self.LEVEL1:
            kpi_set_name = self.kpi_static_data[self.kpi_static_data['kpi_set_fk'] == 31]['kpi_set_name'].values[0]
    ...
    def commit_results_data(self, kpi_set_fk=None):
        ...
        if kpi_set_fk is not None:
            query = CCUS_SANDHolidayQueries.get_atomic_pk_to_delete(self.session_uid, kpi_set_fk)
            kpi_atomic_data = pd.read_sql_query(query, self.rds_conn.db)
            atomic_pks = tuple(kpi_atomic_data['pk'].tolist())
        cur = self.rds_conn.db.cursor()
        if atomic_pks:
            delete_queries = CCUS_SANDHolidayQueries.get_delete_session_results_query(self.session_uid, kpi_set_fk, atomic_pks)
