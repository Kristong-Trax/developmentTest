...
from Projects.CCUS_SAND.Programs.Utils.Fetcher import NEW_CCUS_SANDOBBOQueries
from Projects.CCUS_SAND.Programs.Utils.GeneralToolBox import CCUS_SANDNEW_OBBOGENERALCCUS_SANDToolBox
from Projects.CCUS_SAND.Programs.Utils.ParseTemplates import parse_template
...
TEMPLATE_PATH = os.path.join(os.path.dirname(os.path.realpath(__file__)), '..', 'Data', 'Template_v7.xlsx')
CCNA = 'CCNA'
...
class PROGRAMSCCUS_SANDToolBox:
    LEVEL1 = 1
    LEVEL2 = 2
    LEVEL3 = 3
    ...
    def __init__(self, data_provider, output):
        ...
        self.tools = CCUS_SANDNEW_OBBOGENERALCCUS_SANDToolBox(self.data_provider, self.output, rds_conn=self.rds_conn)
    ...
    def get_kpi_static_data(self):
        ...
        """
        query = NEW_CCUS_SANDOBBOQueries.get_all_kpi_data()
    ...
    def commit_results_data(self, kpi_set_fk=None):
        ...
        if kpi_set_fk is not None:
            query = NEW_CCUS_SANDOBBOQueries.get_atomic_pk_to_delete(self.session_uid, kpi_set_fk)
            kpi_atomic_data = pd.read_sql_query(query, self.rds_conn.db)
            atomic_pks = tuple(kpi_atomic_data['pk'].tolist())
        cur = self.rds_conn.db.cursor()
        if atomic_pks:
            delete_queries = NEW_CCUS_SANDOBBOQueries.get_delete_session_results_query(self.session_uid, kpi_set_fk, atomic_pks)
