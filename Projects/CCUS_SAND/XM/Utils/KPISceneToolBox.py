...
from Projects.CCUS_SAND.XM.Utils.Const import CCUS_SANDConst

__author__ = 'Shivi'


class CCUS_SANDSceneToolBox:
    ...
    def __init__(self, data_provider, output, common):
        ...
        self.kpi_fk = self.common.get_kpi_fk_by_kpi_name(CCUS_SANDConst.POC)
        self.poc_number = 1

    def scene_score(self):
        if self.template_group in CCUS_SANDConst.DICT_WITH_TYPES.keys():
            for poc in self.match_product_in_scene[CCUS_SANDConst.DICT_WITH_TYPES[self.template_group]].unique().tolist():
                relevant_match_products = self.match_product_in_scene[self.match_product_in_scene[
                    CCUS_SANDConst.DICT_WITH_TYPES[self.template_group]] == poc]
