...
class BATRU_SANDPositionGraphs:
    ...
    def get_entity_matrix(self, scene_id, entity, reversed=False):
        """
        ...
        for vertex in graph.vs:
            ...
            if incidents:
                ...
                if reversed:
                    shelf_number_from_bottom_dict[vertex_id] = vertex['shelf_number_from_bottom']
        if not reversed:
            ...
            # matrix.reverse()
            new_matrix = [0]*len(shelf_number_from_bottom_dict.values())
            for row in matrix:
                current = row[0]
                first_current = row[0]
                while current in incidents_dict.keys():
                ...
                new_matrix[shelf_number_from_bottom_dict[first_current]] = row
                matrix = new_matrix
        return matrix

    def transform_entity_matrix_to_df(self, matrix, total_num_of_shelves=None):
        dict_for_df = {}
        shelf_number = 0
        for shelf in matrix:
            shelf_number += 1
            ...
            for product in shelf:
                sequence += 1
                if total_num_of_shelves is None:
                    dict_for_df[shelf_number, sequence] = {'shelf_number': shelf_number, 'product_name': product,
                                                           'sequence': sequence}
                else:
                    shelf_number_from_bottom = int(float(total_num_of_shelves)) - (shelf_number + 1)
                    dict_for_df[shelf_number, sequence] = {'shelf_number': shelf_number, 'product_name': product,
                                                           'sequence': sequence,
                                                           'shelf_number_from_bottom': shelf_number_from_bottom}
