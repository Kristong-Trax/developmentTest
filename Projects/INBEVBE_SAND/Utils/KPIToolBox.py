...
from Projects.INBEVBE_SAND.Utils.Fetcher import INBEVBE_SANDQueries
from Projects.INBEVBE_SAND.Utils.INBEVBEJSON import INBEVBE_SANDJsonGenerator
from Projects.INBEVBE_SAND.Utils.ToolBox import INBEVBE_SANDINBEVToolBox
...
class INBEVBE_SANDToolBox:
    ...
    def __init__(self, data_provider, output):
        ...
        self.tools = INBEVBE_SANDINBEVToolBox(self.data_provider, output,
    ...
    def get_business_unit(self):
        ...
        """
        query = INBEVBE_SANDQueries.get_business_unit_data(self.store_info['store_fk'].values[0])
    ...
    def get_kpi_static_data(self):
        ...
        """
        query = INBEVBE_SANDQueries.get_all_kpi_data()
    ...
    def get_match_display(self):
        ...
        """
        query = INBEVBE_SANDQueries.get_match_display(self.session_uid)
        match_display = pd.read_sql_query(query, self.rds_conn.db)
        return match_display

    def get_osa_table(self):
        query = INBEVBE_SANDQueries.get_osa_table(self.store_id)
        osa_table = pd.read_sql_query(query, self.rds_conn.db)
        return osa_table

    def get_oos_messages(self):
        query = INBEVBE_SANDQueries.get_oos_messages(self.session_uid)
        oos_messages = pd.read_sql_query(query, self.rds_conn.db)
        return oos_messages

    def get_store_number_1(self):
        query = INBEVBE_SANDQueries.get_store_number_1(self.store_id)
    ...
    def get_eye_level_shelves(self, shelves_num):
        jg = INBEVBE_SANDJsonGenerator('inbevbe_sand')
    ...
    def remove_product_from_store_assortment(self, products_list):
        for product in products_list:
            custom_osa_query = INBEVBE_SANDQueries.get_delete_osa_records_query(product, self.store_id, datetime.utcnow())
                    ...
                    for scene in scenes_to_check:
                        ...
                        for bay in bays:
                            ...
                            if scene_recognition_flag:
                                ...
                                # if len(bay_displays) > 1: # todo: no need in calculation of factor
                                ...
                                # except Exception as e:
                                #     factor = 1
                                number_of_shelves = len(bay_data['shelf_number'].unique()) + (int(factor) - 1) # todo: insert the factor according to the right display
                            else:
    ...
    def custom_share_of_shelf(self, set_name):
        ...
        if set_name == 'Linear Share of Shelf vs. Target':
            # store_attribute_8 = self.get_store_attribute_7(self.store_id)
    ...
    def calculate_custom_share_of_shelf(self, set_name, target=None):
        pallet_size_dict = {'Full Pallet': self.PALLET_SIZE_MM, 'Half Pallet': self.HALF_PALLET_SIZE_MM, '1_display': self.HALF_PALLET_SIZE_MM, '2_display': self.HALF_PALLET_SIZE_MM}
        sos_dict={}
        ...
        for scene in scenes_to_check:
            ...
            for bay in bays:
                ...
                bay_displays = bay_match_display_in_scene[
                    bay_match_display_in_scene['name'].isin([PALLET, HALF_PALLET])]['name'].tolist()
                lowest_shelf = max(shelves)
                products_with_zero_length = \
                    filtered_products.loc[filtered_products['shelf_number'] == lowest_shelf][
                        'product_fk'].unique().tolist()
                lowest_shelf_data = matches.loc[(matches['bay_number'] == bay) & (matches['shelf_number'] == lowest_shelf)]
                ...
                if bay_displays:  # todo: to insert the rules in case we recoginzed displays
                    if len(bay_displays) > 1:
                        factors = self.get_product_factor_by_lowest_shelf(lowest_shelf_data, products_to_check,
                                                                          display_unit=bay_displays,
                                                                          display_data=bay_match_display_in_scene)
                        if factors is not None:
                            for display, factor_dict in factors.items():
                                            denominator += self.calculate_denominator_for_pallet(display, factor_dict, pallet_size_dict, scene)
                                            nominator +=self.calculate_nominator_for_pallet(display, factor_dict, pallet_size_dict, scene)
                        ...
                        if bay_displays[0]== HALF_PALLET:
                            if self.check_rule(lowest_shelf_data, most_common_product, HALF_PALLET_STACK_PARAM, HALF_PALLET_SEQUENCE_PARAM):
                                factors = self.get_product_factor_by_lowest_shelf(lowest_shelf_data, products_to_check,  display_unit=bay_displays)
                        elif bay_displays[0]== PALLET:
                            if self.check_rule(lowest_shelf_data, most_common_product, PALLET_STACK_PARAM,
                        ...
                        if factors is None:
                            ...
                            for display, factor_dict in factors.items():
                                    denominator += self.calculate_denominator_for_pallet(display, factor_dict, pallet_size_dict,
                                                                         scene)
                                    nominator += self.calculate_nominator_for_pallet(display, factor_dict, pallet_size_dict,
                                                                         scene)
                                    self.insert_product_to_csif_legth_dict(scene, product=factor_dict.keys()[0],
                                                                           custom_mm_length=pallet_size_dict[display] * int(
                ...
                elif self.check_rule_hidden_pallet(lowest_shelf_data, most_common_product, NON_PALLET_MIN_STACK_PARAM, NON_PALLET_MAX_STACK_PARAM , NON_PALLET_MAX_SEQUENCE_PARAM):
                    lowest_shelf_data=lowest_shelf_data.loc[lowest_shelf_data['product_fk']==most_common_product]
                    lowest_shelf_data_with_manufacturer=pd.merge(lowest_shelf_data, self.all_products, on=['product_fk', 'product_fk'],
                             suffixes=['', '_1'])
                    size = \
                        self.all_products.loc[(self.all_products['product_fk'] == most_common_product)]['width_mm'].values[0]
                    if size is not None and not np.isnan(size):
                        custom_mm_length = self.calculate_custom_mm_length(lowest_shelf_data_with_manufacturer, scene, lowest_shelf,
                                                                           most_common_product, size, width_mm_field=None)
                        nominator_to_add=self.calculate_nominator(lowest_shelf_data_with_manufacturer, scene, lowest_shelf,
                                                                  most_common_product, size, width_mm_field=None)
                        if custom_mm_length>4800:
                            if nominator_to_add!=0:
                                nominator+=4800
                            denominator += 4800
                            custom_mm_length=4800
                        else:
                            nominator+=nominator_to_add
                            denominator +=custom_mm_length
                    else:
                        if any(lowest_shelf_data_with_manufacturer.loc[
                                                   (lowest_shelf_data_with_manufacturer['manufacturer_name'] == self.ABINBEV) & (
                                                       lowest_shelf_data_with_manufacturer['shelf_number'] == lowest_shelf) & (
                                               lowest_shelf_data_with_manufacturer['product_fk'] == most_common_product)][
                                   'width_mm_advance']):
                            width_mm_field = 'width_mm_advance'
                        else:
                            width_mm_field = 'width_mm'
                        custom_mm_length = self.calculate_custom_mm_length(lowest_shelf_data_with_manufacturer, scene, lowest_shelf,
                                                                           most_common_product, size, width_mm_field)

                        nominator_to_add = self.calculate_nominator(lowest_shelf_data_with_manufacturer, scene, lowest_shelf, most_common_product, size,
                                                                      width_mm_field)
                        ...
                        if custom_mm_length > 4800:
                            ...
                            nominator += nominator_to_add
                            denominator += custom_mm_length
                    self.insert_product_to_csif_legth_dict(scene, most_common_product, custom_mm_length)
                    products_with_zero_length.remove(most_common_product)
                    for product in products_with_zero_length:
                        self.scene_item_length_mm_dict[scene, product] = 0
                    shelves.remove(lowest_shelf)

                for shelf in shelves:
                    if shelf==lowest_shelf and count_stacking_lowest_shelf==1:
                        products_in_shelf = \
                        ...
                        for product in products_in_shelf:  # Saving results to custom scene item facts table
                            size = \
                            self.all_products.loc[(self.all_products['product_fk'] == product)]['width_mm'].values[0]
                            if size is not None and not np.isnan(size):
                                nominator += self.calculate_nominator(lowest_shelf_data_with_manufacturer, scene, shelf, product, size, width_mm_field=None)
                                denominator += self.calculate_denominator(lowest_shelf_data_with_manufacturer, scene, shelf, product,
                                                                          size, width_mm_field=None)
                                custom_mm_length = self.calculate_custom_mm_length(lowest_shelf_data_with_manufacturer, scene, shelf,
                                ...
                                if any(filtered_products.loc[
                                    ...
                                    width_mm_field = 'width_mm'
                                nominator += self.calculate_nominator(lowest_shelf_data_with_manufacturer, scene, shelf, product, size,
                                                                      width_mm_field)
                                denominator += self.calculate_denominator(lowest_shelf_data_with_manufacturer, scene, shelf, product,
                                                                          size, width_mm_field)
                                custom_mm_length = self.calculate_custom_mm_length(lowest_shelf_data_with_manufacturer, scene, shelf,
                        ...
                        products_in_shelf = \
                            filtered_products.loc[(filtered_products['shelf_number'] == shelf)][
                                'product_fk'].unique().tolist()
                        for product in products_in_shelf:  # Saving results to custom scene item facts table
                            size = self.all_products.loc[(self.all_products['product_fk'] == product)]['width_mm'].values[0]
                            if size is not None and not np.isnan(size):
                                nominator += self.calculate_nominator(filtered_products, scene, shelf, product, size, width_mm_field=None)
                                denominator += self.calculate_denominator(filtered_products, scene, shelf, product, size, width_mm_field=None)
                                custom_mm_length = self.calculate_custom_mm_length(filtered_products, scene, shelf, product, size, width_mm_field=None)
                            else:
                                if any(filtered_products.loc[
                                    (filtered_products['manufacturer_name'] == self.ABINBEV) & (
                                        filtered_products['shelf_number'] == shelf) & (
                                        filtered_products['product_fk'] == product)]['width_mm_advance']):
                                    width_mm_field = 'width_mm_advance'
                                else:
                                    width_mm_field = 'width_mm'
                                nominator +=self.calculate_nominator(filtered_products, scene, shelf, product, size, width_mm_field)
                                denominator += self.calculate_denominator(filtered_products, scene, shelf, product, size, width_mm_field)
                                custom_mm_length = self.calculate_custom_mm_length(filtered_products, scene, shelf, product, size, width_mm_field)
                            self.insert_product_to_csif_legth_dict(scene, product, custom_mm_length)
        if denominator:
    ...
    def get_product_factor_by_lowest_shelf(self, bay_data, products_to_check, display_unit=[], display_data=None):
        ...
        if factor == 'NO VALUE' or factor is None:
            factor = 1
        factors[display_unit[0]] = {most_common_product: factor}
        return factors
    ...
    def get_store_attribute_8(self, store_fk):
        query = INBEVBE_SANDQueries.get_store_attribute_8(store_fk)
        final_df = pd.read_sql_query(query, self.rds_conn.db)
        value = final_df.values[0][0]
        return value

    def get_all_products(self):
        query = INBEVBE_SANDQueries.get_att3_att4_for_products()
    ...
    def get_product_att4(self, product_fk):
        query = INBEVBE_SANDQueries.get_product_att4(product_fk)
    ...
    #     query = INBEVBE_SANDQueries.get_rect_values_query(self.session_uid)
    ...
    def check_rule(self, lowest_shelf_data, common_product, min_stack_param, min_sequence_param):
        ...
        relevant_bay_data_first_layer=lowest_shelf_data.loc[(lowest_shelf_data['product_fk']==common_product) & (lowest_shelf_data['stacking_layer']==1)]
    ...
    def check_rule_hidden_pallet(self, bay_data, common_product, min_stack_param, max_stack_param, max_sequence_param):
        ...
        if (max(relevant_bay_data['stacking_layer'].values) >= min_stack_param) and (max(relevant_bay_data['stacking_layer'].values) <= max_stack_param):
            if relevant_bay_data_first_layer['product_fk'].size <= max_sequence_param:
                return True
        return False

        # min_sequence_for_product = min(relevant_bay_data['sku_sequence_number'].values())
        # sequence_list=bay_data.loc[bay_data['sku_sequence_number']>=min_sequence_for_product]['sku_sequence_number'].tolist()
        # # max_sequence_for_product=max(relevant_bay_data['sku_sequence_number'].values())
        # # number_of_sequences=max_sequence_for_product-min_sequence_for_product+1
        # for sequence in sequence_list:
        #     if bay_data.loc[bay_data['product_fk']]
        #         number_of_sequences+=1
        # if number_of_sequences>=sequence_param:
        #     if bay_data.loc[(bay_data['product_fk']==common_product)& (bay_data['stacking_layer']==1)]
    ...
    def commit_results_data(self):
        ...
        delete_queries = INBEVBE_SANDQueries.get_delete_session_results_query(self.session_uid)
        pservice_tables_delete_query = INBEVBE_SANDQueries.get_delete_custom_scif_query(self.session_fk)
