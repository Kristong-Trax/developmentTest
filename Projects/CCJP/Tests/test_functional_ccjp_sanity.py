...
from Projects.CCJP.Tests.Data.kpi_results import CCJPKpiResults
# import os
# import json
...
class TestKEnginePsCode(PsSanityTestsFuncs):
    ...
    @PsSanityTestsFuncs.seeder.seed(["ccjp_seed", "mongodb_products_and_brands_seed"], ProjectsSanityData())
    def test_ccjp_sanity(self):
        self.add_mocks()
        project_name = ProjectsSanityData.project_name
        data_provider = KEngineDataProvider(project_name)
        sessions = {'299BA136-7862-4D7D-ABBD-2070F6B3E57E': []}
        kpi_results = CCJPKpiResults().get_kpi_results()
        for session in sessions.keys():
            data_provider.load_session_data(str(session))
            output = Output()
            CCJPCalculations(data_provider, output).run_project_calculations()
