...
from Projects.JTIUA_SAND.Utils.Fetcher import JTIUA_SANDQueries
from Projects.JTIUA_SAND.Utils.GeneralToolBox import JTIUA_SANDGENERALToolBox
from Projects.JTIUA_SAND.Utils.ParseTemplates import parse_template
...
class JTIUA_SANDToolBox:
    ...
    def __init__(self, data_provider, output):
        ...
        self.tools = JTIUA_SANDGENERALToolBox(self.data_provider, self.output, rds_conn=self.rds_conn)
        ...
        self.posm_template = parse_template(TEMPLATE_PATH, 'POSM',
                                            columns_for_vertical_padding=[JTIUA_SANDConsts.SCENE_TYPE,
    ...
    def get_match_display(self):
        ...
        """
        query = JTIUA_SANDQueries.get_match_display(self.session_uid)
    ...
    def get_kpi_static_data(self):
        ...
        """
        query = JTIUA_SANDQueries.get_all_kpi_data()
    ...
    def main_calculation(self):
        ...
        """
        main_score = 0
        main_set = self.template_data[~self.template_data[JTIUA_SANDConsts.KPI_GROUP].apply(bool)].iloc[0]
        pillar_data = self.template_data[self.template_data[JTIUA_SANDConsts.KPI_GROUP] == main_set[JTIUA_SANDConsts.KPI_NAME]]
        for x, pillar in pillar_data.iterrows():
            kpi_data = self.template_data[self.template_data[JTIUA_SANDConsts.KPI_GROUP] == pillar[JTIUA_SANDConsts.KPI_NAME]]
            pillar_score = 0
            for y, params in kpi_data.iterrows():
                kpi_type = params[JTIUA_SANDConsts.KPI_TYPE]
                filters = self.get_filters(params)
                if kpi_type == JTIUA_SANDConsts.LINEAR_SOS:
                    ...
                    kpi_score = self.tools.calculate_linear_share_of_shelf(sos_filters, self.tools.INCLUDE_EMPTY,
                                                                           **filters)
                elif kpi_type == JTIUA_SANDConsts.SHARE_OF_HOTSPOT:
                    kpi_score = self.calculate_linear_sos_within_hotspot(filters)
                elif kpi_type == JTIUA_SANDConsts.SHARE_OF_POSM:
        ...
        main_fk = self.kpi_static_data[self.kpi_static_data['kpi_set_name'] ==
                                       main_set[JTIUA_SANDConsts.KPI_NAME]]['kpi_set_fk'].values[0]
    ...
    def get_filters(self, params):
        ...
        """
        filters = {}
        if params[JTIUA_SANDConsts.INCLUDE_SCENE_TYPES]:
            filters['template_name'] = params[JTIUA_SANDConsts.INCLUDE_SCENE_TYPES].split(JTIUA_SANDConsts.SEPARATOR)
        elif params[JTIUA_SANDConsts.EXCLUDE_SCENE_TYPES]:
            filters['template_name'] = (params[JTIUA_SANDConsts.EXCLUDE_SCENE_TYPES].split(JTIUA_SANDConsts.SEPARATOR),
                                        self.tools.EXCLUDE_FILTER)
        if params[JTIUA_SANDConsts.MANUFACTURER]:
            filters['manufacturer_name'] = params[JTIUA_SANDConsts.MANUFACTURER].split(JTIUA_SANDConsts.SEPARATOR)
    ...
    def calculate_posm_scoring(self):
        ...
        match_display = self.match_display[self.match_display['display_type'].isin(
            self.posm_template[JTIUA_SANDConsts.DISPLAY_TYPE].unique().tolist())]
        for x, posm in match_display.iterrows():
            posm_data = self.posm_template[(self.posm_template[JTIUA_SANDConsts.SCENE_TYPE] == posm['template_name']) &
                                           (self.posm_template[JTIUA_SANDConsts.DISPLAY_BRAND] == posm['manufacturer']) &
                                           (self.posm_template[JTIUA_SANDConsts.DISPLAY_NAME].str.contains(posm['display_name'], regex=False))]
            if not posm_data.empty:
                ...
                if hotspot_data and \
                    ...
                    posm_scoring[posm_manufacturer] += float(posm_data[JTIUA_SANDConsts.IN_HS])
                else:
                    posm_scoring[posm_manufacturer] += float(posm_data[JTIUA_SANDConsts.NOT_IN_HS])
        return posm_scoring
    ...
    def calculate_hotspot_range(self):
        ...
        """
        hotspots = {}
        top, right, bottom, left = JTIUA_SANDConsts.HOTSPOT_RANGE
        if self.store_type.endswith(JTIUA_SANDConsts.KIOSK):
            relevant_scenes = self.scene_info[self.scene_info['template_name'] == JTIUA_SANDConsts.KIOSK]
            if not relevant_scenes.empty:
                for scene in relevant_scenes['scene_fk'].unique():
                    ...
                    hotspot_data = self.match_display[(self.match_display['scene_id'] == scene) &
                                                      (self.match_display['display_type'] == JTIUA_SANDConsts.HOTSPOT)]
            ...
            front_scenes = self.scene_info[self.scene_info['template_name'] == JTIUA_SANDConsts.STORE_FRONT]
            back_scenes = self.scene_info[self.scene_info['template_name'] == JTIUA_SANDConsts.STORE_BACK]
    ...
    def commit_results_data(self):
        ...
        delete_queries = JTIUA_SANDQueries.get_delete_session_results_query(self.session_uid)
