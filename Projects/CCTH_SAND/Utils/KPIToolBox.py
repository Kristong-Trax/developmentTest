...
from Projects.CCTH_SAND.Utils.Fetcher import CCTH_SANDQueries
from Projects.CCTH_SAND.Utils.GeneralToolBox import CCTH_SANDGENERALToolBox
from Projects.CCTH_SAND.Utils.ParseTemplates import CCTH_SANDParseTemplates as ParseTemplates, TEMPLATE_PATH

# from Projects.CCTH_SAND.Utils.Report import Report
...
class CCTH_SANDToolBox(CCTH_SANDConsts):
    LEVEL1 = 1
    LEVEL2 = 2
    LEVEL3 = 3
    ...
    def __init__(self, data_provider, output):
        ...
        self.tools = CCTH_SANDGENERALToolBox(self.data_provider, self.output)
    ...
    def get_segmentation_and_region(self):
        ...
        """
        query = CCTH_SANDQueries.get_segmentation_and_region_data(self.store_id)
        data = pd.read_sql_query(query, self.rds_conn.db).iloc[0]
        return data.segmentation, data.region

    def get_store_data(self):
        query = CCTH_SANDQueries.get_store_data(self.store_id)
    ...
    def get_kpi_static_data(self):
        ...
        """
        query = CCTH_SANDQueries.get_all_kpi_data(self.RED_SCORE)
    ...
    def get_kpi_set_fk(self, kpi_name):
        ...
        The data is taken from static.kpi / static.atomic_kpi / static.kpi_set.
        """
        query = CCTH_SANDQueries.get_kpi_fk(kpi_name)
        kpi_static_data = pd.read_sql_query(query, self.rds_conn.db)
        return kpi_static_data
    ...
    def calculate_cooler_rack(self, types, kpi_name):
        ...
        for type in types.split(','):
            ...
            groups = survey_a[survey_a['selected_option_text'].str.upper() == type.upper()]['group_num'].unique()
    ...
    def build_static_data_and_save(self, order, kpi_set_fk, kpi_names):
        ...
        for kpi_name in kpi_names:
            try:
                self.write_to_db_result_report(kpi_set_fk, match[kpi_name], kpi_name, order)
            except:
                pass
            order += 1
        ...
        for c in xrange(len(childrens)):
            ...
            if kpi == self.BPPC:
                ...
                if sovi['SOVI Cola'][0] is not None:
                    cola_result = str(int(sovi['SOVI Cola'][0])) + '%'
                else:
                    cola_result = None
                if sovi['SOVI Sparkling'][0] is not None:
                    sparkling_result = str(int(sovi['SOVI Sparkling'][0])) + '%'
                else:
                ...
                for result in self.summary:
                    ...
                    if result == 'RED SCORE':
                        self.write_to_db_result_report(kpi_set_fk, str("%.2f" % self.summary[result][0]) + '%', 'RED Score Achieved', 900)
                        self.write_to_db_result_report(kpi_set_fk, str("%.2f" % self.summary[result][1]) + '%', 'RED Score Targeted', 901)
                        self.write_to_db_result_report(kpi_set_fk, str("%.2f" % self.summary[result][2]) + '%', 'RED Score (Outlet)', 902)
    ...
    def commit_results_data(self):
        ...
        delete_queries = CCTH_SANDQueries.get_delete_session_results_query(self.session_uid, self.session_fk)
