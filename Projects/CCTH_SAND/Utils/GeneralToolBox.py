...
from Projects.CCTH_SAND.Utils.PositionGraph import CCTH_SANDPositionGraphs

__author__ = 'Nimrod'


class CCTH_SANDGENERALToolBox:
    ...
    def position_graphs(self):
        if not hasattr(self, '_position_graphs'):
            self._position_graphs = CCTH_SANDPositionGraphs(self.data_provider)
    ...
    def check_survey_answer(self, survey_text, target_answer):
        ...
        if survey_data.empty:
            Log.debug('Survey with {} = {} doesn\'t exist'.format(entity, value))
            return None
