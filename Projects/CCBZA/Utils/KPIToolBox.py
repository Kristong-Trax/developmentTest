...
from Projects.CCBZA.Utils.Common_TEMP import Common
...
from Projects.CCBZA.Utils.Fetcher import CCBZA_Queries
from Projects.CCBZA.Utils.ParseTemplates import parse_template
from Projects.CCBZA.Utils.GeneralToolBox import CCBZA_GENERALToolBox
from Projects.CCBZA.Utils.Errors import DataError
...
class CCBZA_ToolBox:
    ...
    def __init__(self, data_provider, output):
        ...
        self.tools = CCBZA_GENERALToolBox(self.data_provider, self.output)
...
#------------------scene calculations-----------------------------
    ...
    def calculate_availability_competitors_majority(self, atomic_kpi, identifier_result):
        ...
        scene_result = 0
        if filters[GENERAL_FILTERS]['scene_fk']:
            competitor_filters = filters[GENERAL_FILTERS].copy()
            ...
            if competitor_facings:
                ...
                ko_facings_cond1 = self.scif[self.tools.get_filter_condition(self.scif, **ko_filters_cond1)]['facings'].sum()

                ko_filters_cond2 = filters[GENERAL_FILTERS].copy()
                ko_filters_cond2.update(filters['3'])
                ko_filters_cond2.update(filters['2'])
                ko_facings_cond2 = self.scif[self.tools.get_filter_condition(self.scif, **ko_filters_cond2)][
                    'facings'].sum()
...
#----------------------session calculations--------------------------
    ...
    def get_store_data_by_store_id(self):
        ...
        query = CCBZA_Queries.get_store_data_by_store_id(store_id)
        query_result = pd.read_sql_query(query, self.rds_conn.db)
        return query_result

    def get_kpi_result_values_df(self):
        query = CCBZA_Queries.get_kpi_result_values()
        query_result = pd.read_sql_query(query, self.rds_conn.db)
        return query_result

    def get_kpi_score_values_df(self):
        query = CCBZA_Queries.get_kpi_score_values()
        query_result = pd.read_sql_query(query, self.rds_conn.db)
        return query_result
    ...
    def get_planogram_results(self):
        ...
        query = CCBZA_Queries.get_planogram_results(scenes)
    ...
    def get_kpi_static_data(self):
        ...
        """
        query = CCBZA_Queries.get_all_kpi_data()
        kpi_static_data = pd.read_sql_query(query, self.rds_conn.db)
        return kpi_static_data
    ...
    def get_new_kpi_static_data(self):
        ...
        """
        query = CCBZA_Queries.get_new_kpi_data()
        kpi_static_data = pd.read_sql_query(query, self.rds_conn.db)
        return kpi_static_data
