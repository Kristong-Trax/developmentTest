...
from Projects.CCUS_TEST.Utils.PositionGraph import CCUSPositionGraphs
...
class CCUSGENERALToolBox:
    ...
    def get_filter_condition(self, df, **filters):
        ...
        :param filters: These are the parameters which the data frame is filtered by.
                       ...
                       INPUT EXAMPLE (1):   manufacturer_name = ('Diageo', DIAGEOAUCCUSGENERALToolBox.INCLUDE_FILTER)
        ...
        for field in filters.keys():
            if field in df.keys():
                ...
                if not isinstance(value, list):
                    value = [value]
                if '_fk' in field or '_id' in field:
                    value = map(int, value)
                if exclude_or_include == self.INCLUDE_FILTER:
                    ...
                    condition = (df[field].str.contains(value[0], regex=False, flags=re.IGNORECASE))
                    for v in value[1:]:
                        condition |= (df[field].str.contains(v, regex=False, flags=re.IGNORECASE))
                elif exclude_or_include == self.NOT_CONTAIN_FILTER:
                    condition = (~df[field].str.contains(value[0], regex=False, flags=re.IGNORECASE))
                    for v in value[1:]:
                        condition &= (~df[field].str.contains(v, regex=False, flags=re.IGNORECASE))
                else:
