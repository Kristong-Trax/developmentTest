...
from Projects.PNGJP_NEW.Utils.Fetcher import PNGJPQueries
from Projects.PNGJP_NEW.Utils.GeneralToolBox import PNGJPGENERALToolBox
from Projects.PNGJP_NEW.Utils.ParseTemplates import parse_template
...
class PNGJPToolBox(PNGJPConsts):
    ...
    def get_category_scene_types(self):
        ...
        for category in scene_types[self.CATEGORY].unique():
            ...
            #     if (data[self.STORE_TYPE] == self.store_type).any():
            #         data = data[data[self.STORE_TYPE] == self.store_type]
            #     else:
            #         data = data[data[self.STORE_TYPE] == 'All']
    ...
    def calculate_distribution(self, category, params):
        ...
        for product_ean_code in distribution_products:
            product_brand = self.all_products[self.all_products['product_ean_code'] ==
    ...
    def calculation_per_product(self):
        ...
        """
        kpis = self.template_data[self.template_data[self.SUB_CALCULATION] == 'product_ean_code']
    ...
    def calculation_per_brand(self):
        ...
        """
        kpis = self.template_data[self.template_data[self.SUB_CALCULATION] == 'brand_name']
        for x, entity in self.scif.drop_duplicates(subset=['brand_fk']).iterrows():
            for y, params in kpis.iterrows():
                ...
                if kpi_type == self.FACING_COUNT:
                    ...
                    if params[self.MANUFACTURER]:
                        manufacturers = params[self.MANUFACTURER].split(self.SEPARATOR)
    ...
    def calculation_per_manufacturer(self):
        ...
        """
        kpis = self.template_data[self.template_data[self.SUB_CALCULATION] == 'manufacturer_name']
        ...
        for x, entity in self.scif.drop_duplicates(subset=['category_fk']).iterrows():
            for y, params in kpis.iterrows():
                ...
                if kpi_type == self.EMPTY_FACING_SOS:
                    ...
                    for display_type in filtered_sheet[self.POSM_TYPE].unique():
                        displays = filtered_sheet[filtered_sheet[self.POSM_TYPE] == display_type]
                        for display_name in displays[self.POSM_NAME].unique():
                            kpi_name = params[self.KPI_NAME].format(category=entity['category'],
                    ...
                    filtered_sheet = custom_sheet[(custom_sheet[self.CATEGORY] == entity['category']) &
                                                  (custom_sheet[self.store_type].apply(bool))]
                    displays = filtered_sheet[self.POSM_NAME].unique().tolist()
                    ...
                    category_assortment = reference_results['result'].sum()
                    for brand in reference_results['brand'].unique():
                        brand_assortment = reference_results[reference_results['brand'] == brand]['result'].sum()
                        score = 0 if not category_assortment else int((brand_assortment / float(category_assortment)) * 100)
                    ...
                    for brand in reference_results['brand'].unique():
                        ...
                        # P&G Share
                        brand_manufacturer = self.brand_to_manufacturer[self.brand_to_manufacturer['brand_name'] ==
                                                                        brand].iloc[0]['manufacturer_name']
                    ...
                    custom_sheet = self.get_template(params[self.CUSTOM_SHEET])
                    brands_data = custom_sheet[custom_sheet[self.CATEGORY] == entity['category']]
                    for z, brand_data in brands_data.iterrows():
                        result = int(self.tools.calculate_assortment(assortment_entity='brand_name',
                                                                     shelf_number=shelves,
                                                                     brand_name=brand_data[self.BRAND],
                                                                     manufacturer_name=brand_data[self.MANUFACTURER],
                                                                     category_fk=entity['category_fk'],
                                                                     **scene_filters))
                        kpi_name = params[self.KPI_NAME].format(category=entity['category'],
                                                                brand=brand_data[self.BRAND])
                        self.save_result(set_name, kpi_name, result=result)
                    continue
                else:
                    continue
