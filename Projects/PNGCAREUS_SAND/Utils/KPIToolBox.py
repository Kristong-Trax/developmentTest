...
from Projects.PNGCAREUS_SAND.Utils.ParseTemplates import parse_template
from Projects.PNGCAREUS_SAND.Utils.Fetcher import PNGCAREUSQueries
from Projects.PNGCAREUS_SAND.Utils.GeneralToolBox import PNGCAREUSGENERALToolBox
...
class PNGCAREUSToolBox:
    ...
    def __init__(self, data_provider, output):
        ...
        self.tools = PNGCAREUSGENERALToolBox(self.data_provider, self.output, rds_conn=self.rds_conn)
    ...
    def get_kpi_static_data(self):
        ...
        """
        query = PNGCAREUSQueries.get_all_kpi_data()
    ...
    def main_calculation(self, kpi_set_fk, *args, **kwargs):
        ...
        for kpi_name in template_kps['KPI Name'].unique().tolist():
            ...
            if kpi_type == 'block together':
                ...
                self.calculate_sequence_sets(kpi_set_fk, kpi_name, 'count share of display', scene_type)
    ...
    def calculate_block_together(self, kpi_set_fk, kpi_name, tab, scene_type):
        ...
        if kpi_template.empty:
            return None
        filters = {kpi_template['block param']: kpi_template['block value']}
        include_empty = False
        scif_scenes = self.scif.loc[self.scif['template_name'].isin(scene_type)]
        scenes = scif_scenes['scene_id'].unique().tolist()
        score = 0
    ...
    def commit_results_data(self):
        ...
        delete_queries = PNGCAREUSQueries.get_delete_session_results_query(self.session_uid)
