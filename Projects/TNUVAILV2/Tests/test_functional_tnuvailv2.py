...
class TestTnuvailv2(TestFunctionalCase):
    ...
    def test_calculate_assortment_previous_results_empty(self):
        ...
        x = results_df[['fk', 'numerator_id', 'numerator_result', 'denominator_id', 'denominator_result', 'result',
                        'score']]
        self.assertEquals(len(results_df[results_df['fk'] == 2017]), 0)
        self.assertEquals(len(results_df[results_df['fk'] == 2018]), 1)

        self.assertEquals(len(results_df[results_df['fk'] == 2019]), 0)
        self.assertEquals(len(results_df[results_df['fk'] == 2020]), 5)

        expected_list = []
        expected_list.append({'fk': 2018, 'numerator_id': 810, 'numerator_result': 2,
                              'denominator_result': 5, 'score': 0,
                              'result': round(2.0 / 5 * 100, 2)})

        expected_list.append({'fk': 2020, 'numerator_id': 7, 'score': 0, 'denominator_result': 1,
                              'numerator_result': 1, 'result': 2})
        expected_list.append({'fk': 2020, 'numerator_id': 8, 'score': 0, 'denominator_result': 1,
                              'numerator_result': 1, 'result': 2})
        expected_list.append({'fk': 2020, 'numerator_id': 9, 'score': 0, 'denominator_result': 1,
                              'numerator_result': 1, 'result': 2})
        expected_list.append({'fk': 2020, 'numerator_id': 10, 'score': 0, 'denominator_result': 1,
                              'numerator_result': 0, 'result': 1})
        expected_list.append({'fk': 2020, 'numerator_id': 11, 'score': 0, 'denominator_result': 1,
                              'numerator_result': 0, 'result': 1})
    ...
    def test_calculate_assortment_previous_results_exist(self):
        matches, scene = self.create_scif_matches_stitch_groups_data_mocks([2])
        self.mock_get_last_session_oos_results(DataTestUnitTnuva.previous_results_1)
        self.mock_get_oos_reasons_for_session(DataTestUnitTnuva.oos_exclude_res_empty)
        self.mock_session_info_property(DataTestUnitTnuva.session_info_new)
        tool_box = TNUVAILToolBox(self.data_provider_mock, self.output)
        tool_box.common_v2.write_to_db_result = MagicMock()
        ...
        expected_list = []
        expected_list.append({'fk': 2018, 'numerator_id': 810, 'numerator_result': 2,
                              'denominator_result': 5, 'score':  3.0/5 * 100,
                              'result': round(2.0 / 5 * 100, 2)})

        expected_list.append({'fk': 2020, 'numerator_id': 7, 'score': 1, 'denominator_result': 1,
                              'numerator_result': 1, 'result': 2})
        expected_list.append({'fk': 2020, 'numerator_id': 8, 'score': 2, 'denominator_result': 1,
                              'numerator_result': 1, 'result': 2})
        expected_list.append({'fk': 2020, 'numerator_id': 9, 'score': 1, 'denominator_result': 1,
                              'numerator_result': 1, 'result': 2})
        expected_list.append({'fk': 2020, 'numerator_id': 10, 'score': 2, 'denominator_result': 1,
                              'numerator_result': 0, 'result': 1})
        expected_list.append({'fk': 2020, 'numerator_id': 11, 'score': 1, 'denominator_result': 1,
                              'numerator_result': 0, 'result': 1})

        expected_list.append({'fk': 2016, 'numerator_id': 810, 'numerator_result': 2,
                              'denominator_result': 5, 'score': 3.0/5 * 100, 'result': 2.0 / 5 * 100})
