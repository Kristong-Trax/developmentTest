...
class TestTnuvaV2(TestFunctionalCase):
    ...
    def test_general_sos_calculation(self):
        test_case_1 = ((11, 11), {ScifConsts.MANUFACTURER_FK: self.tool_box.own_manufacturer_fk})
        test_case_2 = ((0, 11), {ScifConsts.MANUFACTURER_FK: 999})
        test_case_3 = ((1, 11), {ScifConsts.CATEGORY_FK: 252})
    ...
    def test_category_manufacturer_sos(self):
        ...
        for cat_res in category_results:
            ...
            for res in cat_res:
                cat_fk = res[ScifConsts.CATEGORY_FK]
                if res[ScifConsts.MANUFACTURER_FK] == self.tool_box.own_manufacturer_fk:
                    self.assertEqual(expected_own_manu_in_cat_res[cat_fk], res[SessionResultsConsts.NUMERATOR_RESULT])
    ...
    def compare_store_results(self, lvl3_data, expected_results, is_dist):
        ...
        num_res, den_res = store_res[0][SessionResultsConsts.NUMERATOR_RESULT], store_res[0][SessionResultsConsts.DENOMINATOR_RESULT]
    ...
    def compare_category_results(self, lvl3_data, expected_res, is_dist):
        ...
        for res in cat_results:
            cat_fk, manufacturer_fk = res[ScifConsts.CATEGORY_FK], res[SessionResultsConsts.DENOMINATOR_ID]
            if (cat_fk, manufacturer_fk) in expected_res.keys():
                self.assertEqual(expected_res[(cat_fk, manufacturer_fk)], res[SessionResultsConsts.NUMERATOR_RESULT])
