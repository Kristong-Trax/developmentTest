...
from Projects.INTEG28.Fetcher import INTEG28PNGQueries
...
class INTEG28PNGToolBox:
    ...
    def get_category_name(self, category_fk):
        return self.all_products[self.all_products['category_fk'] == category_fk]['category'].values[0]
    ...
    def get_kpi_static_data(self):
        query = INTEG28PNGQueries.get_all_kpi_data()
        kpi_static_data = pd.read_sql_query(query, self.rds_conn.db)
        return kpi_static_data

    def get_session_categories_data(self):
        query = INTEG28PNGQueries.get_session_categories_query().format(self.session_uid)
    ...
    def main_calculation(self):
        ...
        for category in self.empty_spaces.keys():
            for sub_category in self.empty_spaces[category].keys():
                ...
                main_category = True
                category_facings = relevant_facings[relevant_facings['category'] == category]
    ...
    #     set_data = self.kpi_static_data[self.kpi_static_data['kpi_set_name'] == category]
    #     if sub_category not in set_data['kpi_name'].tolist():
    #         cur = self.rds_conn.db.cursor()
    #         kpi_query = INTEG28PNGQueries.get_insert_kpi_query(set_data['kpi_set_fk'].values[0], sub_category)
    #         cur.execute(kpi_query)
    #         atomic_queries = INTEG28PNGQueries.get_insert_atomic_query(cur.lastrowid)
    ...
    def commit_results_data(self):
        ...
        """
        cur = self.rds_conn.db.cursor()
        delete_queries = INTEG28PNGQueries.get_delete_session_results_query(self.session_uid)
