...
from Projects.PNGAU.Utils.Fetcher import PNGAUQueries
from Projects.PNGAU.Utils.GeneralToolBox import PNGAUGENERALToolBox
from Projects.PNGAU.Utils.EmptySpaces import PNGAUEmptySpaces
from Projects.PNGAU.Utils.ParseTemplates import parse_template
...
class PNGAUToolBox(PNGAUConsts):
    ...
    def __init__(self, data_provider, output):
        ...
        self.tools = PNGAUGENERALToolBox(self.data_provider, self.output, rds_conn=self.rds_conn)
        if self.visit_date <= datetime(2017, 12, 31).date():
            self.TEMPLATE_PATH = os.path.join(os.path.dirname(os.path.realpath(__file__)), '..', 'Data', 'Template Dec.xlsx')
        else:
            self.TEMPLATE_PATH = os.path.join(os.path.dirname(os.path.realpath(__file__)), '..', 'Data', 'Template.xlsx')
    ...
    def commit_results_data(self):
        ...
        delete_queries = PNGAUQueries.get_delete_session_results_query(self.session_uid)
