...
class GOOGLEUS_SANDQueries(object):
    ...
    def get_prices(session_uid):
        return """
            select pd.product_fk, max(ifnull(pd.price, pd.promotion_price)) as price
            from probedata.match_product_in_probe_details pd
            join probedata.probe p on p.pk = pd.probe_fk
            join probedata.scene s on s.pk = p.scene_fk
            where s.session_uid = '{}'
            group by pd.product_fk
        """.format(session_uid)
