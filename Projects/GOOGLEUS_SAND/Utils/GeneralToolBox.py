...
from Projects.GOOGLEUS_SAND.Utils.PositionGraph import GOOGLEUS_SANDPositionGraphs

__author__ = 'Nimrod'


class GOOGLEUS_SANDGENERALToolBox:
    ...
    def position_graphs(self):
        if not hasattr(self, '_position_graphs'):
            self._position_graphs = GOOGLEUS_SANDPositionGraphs(self.data_provider, rds_conn=self.rds_conn)
    ...
    def get_survey_answer(self, survey_data, group_name=None, answer_field=None):
        """
        ...
        survey = self.survey_response[(self.survey_response[entity] == value) &
                                      (self.survey_response[entity2] == value2)]
        if survey.empty:
            return None
    ...
    def get_filter_condition(self, df, **filters):
        ...
        :param filters: These are the parameters which the data frame is filtered by.
                       ...
                       INPUT EXAMPLE (1):   manufacturer_name = ('Diageo', DIAGEOAUGOOGLEUSGOOGLEUS_SANDGENERALToolBox.INCLUDE_FILTER)
