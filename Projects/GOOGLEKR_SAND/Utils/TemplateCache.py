...
from KPIUtils.GlobalProjects.HEINZ.Utils.ParseTemplates import parse_template
...
class HEINZTemplateAsCache:
    ...
    def get_projects_path():
        ...
        for index, name in enumerate(current_path.split('/')):
            if name in ('DeploymentSDK', 'KPIUtils', 'Projects'):
                break
    ...
    def convert_excel_file(self, file_path):
        if self.template_type == self.DATA_FRAME:
            ...
            data = HEINZGENERALToolBox.get_json_data(file_path)
        else:
