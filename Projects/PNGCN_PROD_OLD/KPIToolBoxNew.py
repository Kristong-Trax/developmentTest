...
class PNGToolBox:
    ...
    def get_match_product_in_scene(self):
        ...
        matches = matches.merge(self.data_provider[Data.MATCHES], how='left', on='scene_match_fk')
        matches = matches.merge(self.all_products, how='left', on='product_fk')
        matches = matches.merge(self.scif[['product_fk', 'scene_fk', 'rlv_sos_sc', 'location_type']],
                                how='left', on=['product_fk', 'scene_fk'])
    ...
    def main_calculation(self):
        ...
        for category in self.empty_spaces.keys():
            kpi_dict = {}
            empty_spaces = self.empty_spaces[category]
            ...
            for kpi in kpi_dict.keys():
                kpi_data = set_data[set_data['kpi_name'] == kpi]
