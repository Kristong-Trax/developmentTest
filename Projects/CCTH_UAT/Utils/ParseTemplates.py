...
class CCTH_UATGapConsts(object):
...
class CCTH_UATSurveyConsts(object):
...
class CCTH_UATAvailabilityConsts(object):
...
class CCTH_UATKPIConsts(object):
...
class CCTH_UATParseTemplates(CCTH_UATKPIConsts):
    ...
    def __init__(self, template):
        ...
        self.survey_consts = CCTH_UATSurveyConsts()
        self.availability_consts = CCTH_UATAvailabilityConsts()
        self.gap_consts = CCTH_UATGapConsts()
    ...
    @strip_df
    def parse_gap(self):
        if self.template in (self.TEMPLATE_TT_UNTIL_NOV2017, self.TEMPLATE_TT_AFTER_NOV2017):
        ...
        elif self.template in (self.TEMPLATE_7_11_UNTIL_JULY2017, self.TEMPLATE_7_11_AFTER_JULY2017):
    ...
    def parse_survey(self):
        if self.template in (self.TEMPLATE_TT_UNTIL_NOV2017, self.TEMPLATE_TT_AFTER_NOV2017):
            ...
            for c in xrange(starting_column, len(columns)):
                column = columns[c]
                data = template_data[column]
                region, store_type, segmentation = regions_columns[c], store_types_columns[c].split('.')[0], \
                                                   column.split('.')[0]
                for r in region.split(self.SEPARATOR):
