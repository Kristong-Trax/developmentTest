...
class GOOGLEKRSceneGOOGLEKRGOOGLEToolBox:
    ...
    def google_global_SOS(self):
        scif = self.filter_df(self.scif.copy(), GOOGLEKRConst.EXCLUDE_FILTERS, exclude=1)
        brands_scif = scif[~scif[GOOGLEKRConst.BRAND].isin(GOOGLEKRConst.NOTABRAND)]
        if brands_scif.empty:
            return
        denominators = {
            GOOGLEKRConst.SOS_OUT_OF_SCENE: scif[GOOGLEKRConst.FACINGS].sum(),
            GOOGLEKRConst.SOS_IN_SCENE: brands_scif[GOOGLEKRConst.FACINGS].sum()
        }
        brand_totals = brands_scif.set_index(['brand_fk', GOOGLEKRConst.BRAND])\
                                  .groupby([GOOGLEKRConst.BRAND, 'brand_fk'])[GOOGLEKRConst.FACINGS]\
                                  .sum()

        for (brand_name, brand_fk), numerator in brand_totals.iteritems():
            for kpi in GOOGLEKRConst.SOS_KPIs:
    ...
    def get_planogram_fixture_details(self):
        fixture_kpi_fk = self.common.get_kpi_fk_by_kpi_name(GOOGLEKRConst.FIXTURE_POG)
        compliance_kpi_fk = self.common.get_kpi_fk_by_kpi_name(GOOGLEKRConst.FIXTURE_COMPLIANCE)
        ...
        for i in range(1, 5):
            ...
            self.common.write_to_db_result(
                ...
                score=ratio, by_scene=True, identifier_parent=GOOGLEKRConst.FIXTURE_POG)
            if i == 3:
                self.common.write_to_db_result(
                    fk=fixture_kpi_fk, numerator_id=planogram_id, numerator_result=status_products,
                    denominator_result=all_facings,
                    score=ratio, by_scene=True, identifier_result=GOOGLEKRConst.FIXTURE_POG)

    def get_compliance_products(self, compliance_status_fk, identifier_parent):
        kpi_fk = self.common.get_kpi_fk_by_kpi_name(GOOGLEKRConst.POG_PRODUCT)
    ...
    def get_fixture_osa(self):
        ...
        osa_kpi_fk = self.common.get_kpi_fk_by_kpi_name(GOOGLEKRConst.FIXTURE_OSA)
        ...
        missings_kpi_fk = self.common.get_kpi_fk_by_kpi_name(GOOGLEKRConst.MISSING_DENOMINATIONS)
    ...
    def filter_df(df, filters, exclude=0):
        for exclude_filter in filters:
            if exclude:
                df = df[~df['product_type'].isin(GOOGLEKRConst.EXCLUDE_FILTERS[exclude_filter])]
            else:
                df = df[df['product_type'].isin(GOOGLEKRConst.EXCLUDE_FILTERS[exclude_filter])]
        return df
