...
from KPIUtils_v2.Utils.Consts.DataProvider import MatchesConsts, ProductsConsts, ScifConsts, SessionInfoConsts, \
TemplatesConsts
...
from KPIUtils_v2.Utils.Consts.GlobalConsts import HelperConsts, BasicConsts, ProductTypeConsts
from KPIUtils_v2.GlobalDataProvider.PsDataProvider import PsDataProvider
from KPIUtils_v2.Calculations.AssortmentCalculations import Assortment
from Trax.Utils.Logging.Logger import Log
from KPIUtils.GlobalProjects.GSK.Utils.Const import Const
...
class GSKToolBox:
    ...
    def __init__(self, data_provider, output, common, set_up_file):
        ...
        self.set_up_data = {("Facings SOS", Consts.KPI_TYPE_COLUMN): Consts.NO_INFO, ("Linear SOS", Consts.KPI_TYPE_COLUMN):
                            Consts.NO_INFO, ("Availability",
                                             Consts.KPI_TYPE_COLUMN): Consts.NO_INFO}
    ...
    def extract_data_set_up_file(self, kpi_type, set_up_data=None, kpi_dict=None):
        """
        The function receives kpi type : "GSK_FSOS", "GSK_LSOS", "availability"
    ...
    def main_sos_calculation(self, kpi_type, sos_type):
        ...
        df = self.initialize_sos_df(sos_type)
        if df is None:
            return
    ...
    def initialize_sos_df(self, sos_type):
        """
            This function preforms all changes needed in scif in order to have full info to sos calculation .
            :param sos_type: "FSOS" /"LSOS"
            :returns df if df not empty after filtering and changes else empty
        """
    ...
    def tests_by_template(self, kpi_type, df, set_up_data=None):
        ...
        """
        :param  kpi_type : "GSK_FSOS", "GSK_LSOS", "availability"
    ...
    def assortment_calculation(self, assort_lvl3, denominator_fk, availability_type):
        ...
        if not assort_lvl3.empty:
            ...
            for result in lvl2_result.itertuples():
                ...
                res = round(np.divide(float(result.passes), float(denominator_res)), 4)
                if res >= 1:
                    score = 1
                else:
                    score = 0
    ...
    def assortment_lvl3_gsk_adjustments1(self):
        ...
        """
        The function takes dataframe of level 3 assortment and manipulate the information :
         remove substation products from assortment
         adding products in scif  that there manufacture is GSK
        """
        ...
        if filtered_scif is not None:
            ...
            lvl3_result.drop_duplicates(AssortmentProductConsts.PRODUCT_FK, inplace=True)  # when in 2 different scenes I see the same product
