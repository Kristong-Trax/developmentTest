...
class HOLIDAYToolBox:
    ...
    def __init__(self, data_provider, output):
        ...
        self.tools = HolidayGENERALToolBox(self.data_provider, self.output, rds_conn=self.rds_conn)
    ...
    def get_kpi_static_data(self):
        ...
        """
        query = HolidayQueries.get_all_kpi_data()
    ...
    def commit_results_data(self, kpi_set_fk=None):
        ...
        if kpi_set_fk is not None:
            query = HolidayQueries.get_atomic_pk_to_delete(self.session_uid, kpi_set_fk)
            kpi_atomic_data = pd.read_sql_query(query, self.rds_conn.db)
            atomic_pks = tuple(kpi_atomic_data['pk'].tolist())
        cur = self.rds_conn.db.cursor()
        if atomic_pks:
            delete_queries = HolidayQueries.get_delete_session_results_query(self.session_uid, kpi_set_fk, atomic_pks)
