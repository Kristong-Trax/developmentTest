...
from Projects.PNGCAREUS.Utils.PositionGraph import PNGCAREUSPNGCAREUSPositionGraphs
import networkx as nx
__author__ = 'Nimrod'


class PNGCAREUSPNGCAREUSGENERALToolBox:
    EXCLUDE_FILTER = 0
    INCLUDE_FILTER = 1
    CONTAIN_FILTER = 2
    EXCLUDE_EMPTY = False
    ...
    def position_graphs(self):
        if not hasattr(self, '_position_graphs'):
            self._position_graphs = PNGCAREUSPNGCAREUSPositionGraphs(self.data_provider, rds_conn=self.rds_conn)
            ...
            for shelf in matches['shelf_number'].unique():
                ...
                if not shelf_matches.empty:
                    ...
                    if position:
                        ...
                        if position == 'right':
                            edge_facings = edge_facings.append(shelf_matches.iloc[len(edge_facings) - 1])
                    else:
                        edge_facings = edge_facings.append(shelf_matches.iloc[0])
    ...
    def validate_block_moves(moves, direction_data):
        ...
        if one_to_pass:
            if len(one_to_pass.values()) == 1:
                moves.pop(one_to_pass.values()[0])
                for direction in moves.keys:
    ...
    def calculate_block_together(self, allowed_products_filters=None, include_empty=EXCLUDE_EMPTY,
                                 ...
                                 orphan=False, group=False, block_products=None, group_products=None, **filters):
        """
                        ...
                        if minimum_block_ratio == 1:
                            ...
                            if vertical:
                                G = nx.from_edgelist([(names[x[0]], names[x[1]])
                                                      for names in [block_graph.vs['name']]
                                                      for x in block_graph.get_edgelist()], nx.DiGraph())
                            ...
                            if horizontal:
                                ...
                                direction_data = {'top': (0, 0), 'bottom': (0, 0), 'left': (2, 2), 'right': (2, 2)}
                                result = self.calculate_relative_in_block_per_graph(block_graph, direction_data)
                                results['horizontal'] = result
                            if orphan:
                                orphan_clusters = clusters.pop(cluster)
                                direction_data = {'top': (3, 1000), 'bottom': (3, 1000), 'left': (3, 1000),
                                                  'right': (3, 1000)}
                                results['orphan'] = False
                                for orphan in orphan_clusters:
                                    non_orphan_cluster_vertices = all_vertices.difference([cluster,orphan])
                                    orphan_graph = scene_graph
                                    orphan_graph.delete_vertices(non_orphan_cluster_vertices)
                                    result = self.calculate_relative_in_block_per_graph(scene_graph, direction_data,
                                                                                        cluster, orphan)
    ...
    def get_filter_condition(self, df, **filters):
        ...
        :param filters: These are the parameters which the data frame is filtered by.
                       ...
                       INPUT EXAMPLE (1):   manufacturer_name = ('Diageo', DIAGEOAUPNGCAREUSPNGCAREUSGENERALToolBox.INCLUDE_FILTER)
