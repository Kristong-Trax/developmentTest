...
class INBEVBEINBEVBEToolBox:
    ...
    def __init__(self, data_provider, output):
        ...
        self.tools = INBEVBEINBEVBEINBEVToolBox(self.data_provider, output,
    ...
    def save_level2_and_level3(self, set_name, kpi_name, result, score=None, threshold=None, level_2_only=False,
        ...
        try:
            ...
            Log.info('KPI {} is not defined in the DB for set {}'.format(
                kpi_name.encode('utf-8'), set_name))
        ...
        for scene in scenes_to_check:
            ...
            for bay in bays:
                ...
                if bay_displays:  # todo: to insert the rules in case we recoginzed displays
                    ...
                    if len(bay_displays) > 1:  # 2 pallets
                    ...
                    if factors:
                        ...
                        #         if not (scene, product) in self.scene_item_length_mm_dict.keys():
    ...
    def calculate_nominator(self, shelf_data, scene, shelf, product, size, width_mm_field):
        ...
        if size is not None and not np.isnan(size):
            ...
            nominator = (shelf_data.loc[(shelf_data['manufacturer_name'] == self.ABINBEV) & (
                shelf_data['shelf_number'] == shelf) & (shelf_data['product_fk'] == product)][
                width_mm_field].sum()) * \
                (self.scif.loc[(self.scif['scene_id'] == scene) & (self.scif['product_fk'] == product)][
                    'rlv_sos_sc'].values[0])
        return nominator
    ...
    def get_product_factor_by_lowest_shelf(self, bay_data, products_to_check, display_unit=[],
                                           display_data=None):
        products_sos_dict = {}
        factors = {}
    ...
    def list_by_most_common(lst):
        ...
        counter_by_order = collect.OrderedDict(
    ...
    def shelf_impact_score(self):
        ...
        self.save_level2_and_level3(self.SHELF_IMPACT_SCORE, 'Must Have Assortment',
                                    result=must_have_assortment_points,
                                    level_2_only=True)
    ...
    def calculate_must_have_assortment(self):
        ...
        eans_from_file = [x for x in relevant_store_df['EAN'].unique().tolist()]
        ean_group_dict = {}
        for ean in relevant_store_df['EAN'].unique().tolist():
            ean_group_dict[ean] = relevant_store_df[relevant_store_df['EAN']
                                                    == ean]['ParentProductSetID'].values[0]
        ass_prod_list = self.all_products.loc[
            self.all_products['product_ean_code'].isin(eans_from_file)][
            'product_fk'].unique().tolist()
        groups_list = relevant_store_df['ParentProductSetID'].unique().tolist()
        ...
        for scene in self.scif['scene_fk'].unique().tolist():
            ...
            for product in set(scene_products):
                ...
                if product in ass_prod_list:
                    product_ean_code = self.all_products[self.all_products['product_fk'] == product][
                        'product_ean_code'].values[0]
                    product_group = ean_group_dict[product_ean_code]
                    if product_group in group_dist_dict:
                        existing_dist = group_dist_dict[product_group]
                        group_dist_dict[product_group] = max(dist_score, existing_dist)
                    else:
                        group_dist_dict[product_group] = dist_score
        ...
        if updated_ass_prod_list:
            ...
            set_score = (sum(group_dist_dict.values()) / float(len(groups_list))) * 100
            target = len(groups_list)
        ...
        for scene in scenes_to_check:
            ...
            scene_products_to_check = self.all_products.loc[
                (self.all_products['product_fk'].isin(scene_products)) & (
                    ...
                    ((self.all_products['att1'].isnull()) &
                     (self.all_products['att3'].isnull())))]['product_fk'].unique().tolist()
            missing_bundle_leads = []
    ...
    def calculate_pallet_presence(self):
        """
        This function calculates every Pallet-Presence typed KPI from the relevant sets,
        and returns the set final score.
        """
        pallet_score = len(
    ...
    def calculate_share_of_assortment(self):
        """
        This function calculates every Share-of-Assortment typed KPI from the relevant sets,
        and returns the set final score.
        """
    ...
    def get_missing_products_to_api_set(self, set_name):
        existing_skus = \
            self.all_products[self.all_products['product_type']
                              == 'SKU']['product_ean_code'].unique().tolist()
        set_data = \
            self.kpi_static_data[self.kpi_static_data['kpi_set_name']
