...
class CCRUFT2018Calculations:
    ...
    def __init__(self, data_provider, output, ps_data_provider):
        ...
        self.tool_box = CCRUKPIToolBox(self.data_provider, self.output, ps_data_provider, FT2018)
        self.results = {}
    ...
    def main_function(self):
        ...
        jg.create_json('FT PoS 2018.xlsx', FT2018)
        ...
        score += self.tool_box.calculate_number_of_scenes_no_tagging(
            jg.project_kpi_dict.get('kpi_data')[0])
        attributes_for_table1 = pd.DataFrame([(FT2018, self.session_uid,
                                               self.store_id, self.visit_date.isoformat(), format(score, '.2f'), None)], columns=['kps_name',
                                                                                                                                  ...
                                                                                                                                  'score_1',
                                                                                                                                  'kpi_set_fk'])
        self.tool_box.write_to_db_result(attributes_for_table1, 'level1', FT2018)

        self.tool_box.prepare_hidden_set(jg.project_kpi_dict.get('kpi_data')[0])

        jg.create_gaps_json('gaps_guide_2018.xlsx', sheet_name=FT2018)
        ...
        extra_sets_to_calculate = [(TARGET_EXECUTION, 'Target Execution 2018', TARGET_EXECUTION),
                                   ...
                                   (SPIRITS, 'FT Spirits 2018', 'Sheet1')]
        ...
        jg.create_equipment_json('Contract Execution 2018.xlsx', FT2018)
