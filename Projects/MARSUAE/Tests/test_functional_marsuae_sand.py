...
from Projects.MARSUAE.Tests.data_test_unit_marsuae_sand import DataTestUnitMarsuae
...
def get_test_case_template_all_tests():
    test_case_xls_object = pd.ExcelFile(DataTestUnitMarsuae.test_case_1)
    return test_case_xls_object


def get_external_targets_all_tests():
    external_targets = pd.read_excel(DataTestUnitMarsuae.external_targets)
...
class TestMarsuae(TestFunctionalCase):
    ...
    EXTERNAL_TARGETS = get_external_targets_all_tests()
    SCIF, MATCHES, PROBE_GROUPS = get_scif_matches_stich_groups(TEST_CASE)

    @property
    def import_path(self):
        return 'Projects.MARSUAE.Utils.KPIToolBox'
    ...
    def mock_kpi_external_targets_data(self):
        external_targets_df = self.EXTERNAL_TARGETS
        external_targets = self.mock_object('MARSUAEToolBox.get_all_kpi_external_targets')
    ...
    def mock_kpi_result_value_table(self):
        kpi_result_value = self.mock_object('MARSUAEToolBox.get_kpi_result_values_df')
    ...
    def mock_store_data(self):
        store_data = self.mock_object('MARSUAEToolBox.get_store_data_by_store_id')
    ...
    def mock_probe_group(self, data):
        probe_group = self.mock_object('MARSUAEToolBox.get_probe_group')
    ...
    def create_scif_matches_stitch_groups_data_mocks(self, scenes_list):
        ...
        probe_group = self.mock_probe_group(self.PROBE_GROUPS)
