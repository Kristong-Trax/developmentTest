...
from Projects.PNGJP_SAND.Utils.Fetcher import PNGJP_SANDQueries
from Projects.PNGJP_SAND.Utils.GeneralToolBox import PNGJP_SANDGENERALToolBox
from Projects.PNGJP_SAND.Utils.ParseTemplates import parse_template
...
class PNGJP_SANDToolBox(PNGJP_SANDConsts):
    LEVEL1 = 1
    LEVEL2 = 2
    LEVEL3 = 3
    ...
    def __init__(self, data_provider, output):
        ...
        self.tools = PNGJP_SANDGENERALToolBox(self.data_provider, self.output, rds_conn=self.rds_conn)
        self.template_data = parse_template(TEMPLATE_PATH, 'KPIs')
        self.innovation_assortment = parse_template(TEMPLATE_PATH, 'Innovation Assortment')
        self.psku_assortment = parse_template(TEMPLATE_PATH, 'PSKU Assortment')
    ...
    def get_kpi_static_data(self):
        ...
        """
        query = PNGJP_SANDQueries.get_all_kpi_data()
    ...
    def get_match_display(self):
        ...
        """
        query = PNGJP_SANDQueries.get_match_display(self.session_uid)
    ...
    def main_calculation(self):
        ...
        for category_fk in self.categories:
            category = self.all_products[self.all_products['category_fk'] == category_fk]['category'].values[0]
    ...
    def calculate_share_of_display(self, category, params):
        ...
        try:
            ...
            if 'brand' in params[self.KPI_NAME]:
                brands_to_write = self.scif.loc[self.scif['category'] == category]['brand_name'].unique().tolist()
        ...
        if params[self.MANUFACTURERS]:
            ...
            for item in self.atomic_results[reference_kpi_name].keys():
                ...
                if ((self.all_products['manufacturer_name'].isin(params[self.MANUFACTURERS].split(self.SEPARATOR))) &
                        (self.all_products['brand_name'] == brand)).any():
    ...
    def calculate_distribution(self, category, params):
        ...
        if params[self.CUSTOM_SHEET]:
            ...
            try:
                ...
                distribution_products = distribution_products['product_ean_code'].unique().tolist()
            except KeyError as e:
                distribution_products = []
        else:
            distribution_products = self.scif[(self.scif['category'] == category) &
                                              (self.scif['dist_sc'] == 1)]['product_ean_code'].unique().tolist()
        ...
        for product_ean_code in distribution_products:
            try:
                product_brand = self.all_products.loc[self.all_products['product_ean_code'] ==
                                                      product_ean_code]['brand_name'].values[0]
    ...
    def calculate_distribution_by_scene(self, category, params):
        ...
        if params[self.CUSTOM_SHEET]:
            ...
            distribution_products = self.scif[(self.scif['category'] == category) & (self.scif['dist_sc'] == 1)]
        scenes_filters = self.get_scenes_filters(params, category)
        for product_ean_code in distribution_products['product_ean_code'].unique().tolist():
            product_brand, manufacturer = self.all_products.loc[self.all_products['product_ean_code'] ==
                                          product_ean_code][['brand_name', 'manufacturer_name']].values[0]
        ...
        for p in xrange(len(self.template_data)):
            ...
            if kpi_name not in self.atomic_results.keys():
                ...
                if result > 0 or score > 0:
                    ...
                    self.create_attributes_dict(score=result, level=self.LEVEL3, kpi_set_name=set_name, kpi_name=set_name,
                                            atomic_kpi_name=kpi_name, level3_score=score, threshold=total_calculated)
    ...
    def calculation_per_entity(self, category):
        ...
        for sub_entity in template_data[self.SUB_CALCULATION].unique():
            ...
            if sub_entity in self.scif.keys():
                ...
                # entity_items = self.scif[self.scif['category'] == category][sub_entity].unique().tolist()
            else:
            ...
            for item in entity_items:
                ...
                for p in xrange(len(entity_kpis)):
                    ...
                    if kpi_type == self.FACING_COUNT:
                        ...
                        template_kpi_filters = scenes_filters['template_name']
                        if sub_entity == 'brand_name':
                            brand = item
                            score = None
                            manufacturer = self.all_products.loc[self.all_products['brand_name'] == brand][
                                'manufacturer_name'].values[0]
                        elif sub_entity == 'product_ean_code':
                            brand, manufacturer = self.all_products.loc[self.all_products['product_ean_code'] == item][
                                ['brand_name', 'manufacturer_name']].values[0]
                        ...
                        result = int(
                            self.tools.calculate_assortment(assortment_entity='brand_name',
                        ...
                        if params[self.MANUFACTURERS].strip():
                            kpi_filter = dict({'manufacturer_name': params[self.MANUFACTURERS]}, **kpi_filter)
                        ...
                        if set_name == 'Number of Display (by Brand)':
                            ...
                            scenes_to_check = self.scif.loc[(self.scif['category'] == category) &
    ...
    def commit_custom_scif(self):
        ...
        delete_query = PNGJP_SANDQueries.get_delete_session_custom_scif(self.session_fk)
    ...
    def commit_results_data(self):
        ...
        """
        cur = self.rds_conn.db.cursor()
        delete_queries = PNGJP_SANDQueries.get_delete_session_results_query(self.session_uid)
