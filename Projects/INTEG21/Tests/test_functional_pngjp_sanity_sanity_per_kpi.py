...
from Projects.INTEG21.Tests.Data.test_data_pngjp_sanity_per_kpi import INTEG21ProjectsSanityData
from Trax.Apps.Core.Testing.BaseCase import TestFunctionalCase, MagicMock

from Tests.TestUtils import remove_cache_and_storage
import pandas as pd
from Projects.INTEG21.Tests.Data import parse_template_data
from Projects.INTEG21.Tests.Data import expected_results_from_db

__author__ = 'avrahama'


class INTEG21TestPngjpSanityPerKPI(TestFunctionalCase):
    seeder = Seeder()

    def set_up(self):
        super(INTEG21TestPngjpSanityPerKPI, self).set_up()
        # mock parse_template to return the expected DFs
        self.get_template_path_mock = MagicMock('parse_template',
                                                path='Projects.INTEG21.Utils.ParseTemplates',
                                                side_effect=[
    ...
    @staticmethod
    @seeder.seed(["mongodb_products_and_brands_seed", "pngjp_seed"], INTEG21ProjectsSanityData())
    def save_kpi_results_in_seed():
        """
        load the session and save the results in the seed results for the KPIs
        """
        project_name = INTEG21ProjectsSanityData.project_name
