...
class INTEG21INBEVBEQueries:
    ...
    def get_osa_table(store_fk):
        return """
            SELECT * FROM pservice.custom_store_assortments_products
            WHERE store_fk = {} AND end_date is null AND assortment_fk = 1
        """.format(store_fk)

    @staticmethod
    def get_must_have_table(store_fk):
        return """
            SELECT * FROM pservice.custom_store_assortments_products
            WHERE store_fk = {} AND end_date is null AND assortment_fk = 2
        """.format(store_fk)

    @staticmethod
    def get_delete_osa_records_query(product_fk, store_fk, delete_time, assortment_fk):
        return """
            UPDATE pservice.custom_osa
            SET end_date = '{}'
            WHERE product_fk = {} AND store_fk = {} and assortment_fk= {}
        """.format(delete_time, product_fk, store_fk, assortment_fk)
