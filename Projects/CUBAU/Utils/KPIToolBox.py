...
class CUBAUCUBAUToolBox:
    ...
    def __init__(self, data_provider, output):
        ...
        self.tools = CUBAUCUBAUGENERALToolBox(self.data_provider, self.output, rds_conn=self.rds_conn)
    ...
    def all_param_out_category(self, param_1, param_2, sos_type, product_type, exclude_scene=None, context=None):
        ...
        for i, row in relevant_df.iterrows():
            ...
            if context:
                self.calculate_sos(sos_type, kpi_fk, row[param_1], row[param_2], subset_filters, pop_filters, row[context])
            else:
