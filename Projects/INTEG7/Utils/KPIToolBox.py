...
from Projects.INTEG7.Utils.Fetcher import INTEG7Queries
from Projects.INTEG7.Utils.ToolBox import DIAGEOToolBox
...
class INTEG7ToolBox:
    ...
    def __init__(self, data_provider, output):
        ...
        self.store_channel = self.store_info['store_type'].values[0].upper()
    ...
    def get_business_unit(self):
        ...
        """
        query = INTEG7Queries.get_business_unit_data(self.store_info['store_fk'].values[0])
        return pd.read_sql_query(query, self.rds_conn.db)['name'].values[0]
    ...
    def get_kpi_static_data(self):
        ...
        """
        query = INTEG7Queries.get_all_kpi_data()
    ...
    def get_match_display(self):
        ...
        """
        query = INTEG7Queries.get_match_display(self.session_uid)
    ...
    def calculate_brand_pouring_sets(self, set_name):
        ...
        for params in self.set_templates_data[set_name]:
            if self.tools.calculate_number_of_scenes(**{self.tools.BRAND_POURING_FIELD: 'Yes'}) > 0:
                # 'Pouring' scenes
        ...
        set_score = 0 if not scores else (sum(scores) / float(len(scores))) * 100
        return set_score
    ...
    def calculate_posm_sets(self, set_name):
        ...
        scores = []
        for params in self.set_templates_data[set_name]:
            if self.store_channel == 'OFF TRADE':
    ...
    def commit_results_data(self):
        ...
        delete_queries = INTEG7Queries.get_delete_session_results_query(self.session_uid)
