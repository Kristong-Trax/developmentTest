...
from Projects.INBEVFR.Utils.Fetcher import INBEVFR_PRODINBEVBEQueries
from Projects.INBEVFR.Utils.INBEVFRJSON import INBEVFR_PRODINBEVBEJsonGenerator
from Projects.INBEVFR.Utils.ToolBox import INBEVFR_PRODINBEVBEINBEVToolBox
...
class INBEVFR_PRODINBEVBEToolBox:
    LEVEL1 = 1
    LEVEL2 = 2
    LEVEL3 = 3
    ...
    def __init__(self, data_provider, output):
        ...
        self.tools = INBEVFR_PRODINBEVBEINBEVToolBox(self.data_provider, output,
        ...
        self.product_blocking_score_range = {(0, 60): 0, (60, 80): 10, (80, 101): 15}
    ...
    def get_business_unit(self):
        ...
        """
        query = INBEVFR_PRODINBEVBEQueries.get_business_unit_data(self.store_info['store_fk'].values[0])
    ...
    def get_kpi_static_data(self):
        ...
        """
        query = INBEVFR_PRODINBEVBEQueries.get_all_kpi_data()
    ...
    def get_match_display(self):
        ...
        """
        query = INBEVFR_PRODINBEVBEQueries.get_match_display(self.session_uid)
        match_display = pd.read_sql_query(query, self.rds_conn.db)
        return match_display

    def get_osa_table(self):
        query = INBEVFR_PRODINBEVBEQueries.get_osa_table(self.store_id)
        osa_table = pd.read_sql_query(query, self.rds_conn.db)
        return osa_table

    def get_oos_messages(self):
        query = INBEVFR_PRODINBEVBEQueries.get_oos_messages(self.session_uid)
        oos_messages = pd.read_sql_query(query, self.rds_conn.db)
        return oos_messages

    def get_store_number_1(self):
        query = INBEVFR_PRODINBEVBEQueries.get_store_number_1(self.store_id)
    ...
    def get_eye_level_shelves(self, shelves_num):
        jg = INBEVFR_PRODINBEVBEJsonGenerator('inbevbe')
    ...
    def remove_product_from_store_assortment(self, products_list):
        for product in products_list:
            custom_osa_query = INBEVFR_PRODINBEVBEQueries.get_delete_osa_records_query(product, self.store_id, datetime.utcnow())
            ...
            for display in displays:
                ...
                for product in products_to_check:
                    if self.check_rule(bay_data, product, HALF_PALLET_STACK_PARAM ,HALF_PALLET_SEQUENCE_PARAM):
                        ...
                        if passed_rule>=2:
                            # products_for_two_half_pallets['1_display'] = first_common_product
                            # products_for_two_half_pallets['2_display'] = second_common_product
                            break
                if passed_rule==0:
                    return None
                if passed_rule==1:
    ...
    def get_store_attribute_7(self, store_fk):
        query = INBEVFR_PRODINBEVBEQueries.get_store_attribute_7(store_fk)
        final_df = pd.read_sql_query(query, self.rds_conn.db)
        value = final_df.values[0][0]
        return value

    def get_store_attribute_10(self, store_fk):
        query = INBEVFR_PRODINBEVBEQueries.get_store_attribute_10(store_fk)
        final_df = pd.read_sql_query(query, self.rds_conn.db)
        value = final_df.values[0][0]
        return value

    def get_all_products(self):
        query = INBEVFR_PRODINBEVBEQueries.get_att3_att4_for_products()
    ...
    def get_product_att4(self, product_fk):
        query = INBEVFR_PRODINBEVBEQueries.get_product_att4(product_fk)
        att4 = pd.read_sql_query(query, self.rds_conn.db)
        return att4.values[0][0]

    def get_rect_values(self):
        query = INBEVFR_PRODINBEVBEQueries.get_rect_values_query(self.session_uid)
        rect_values = pd.read_sql_query(query, self.rds_conn.db)
        return rect_values
    ...
    def commit_results_data(self):
        ...
        delete_queries = INBEVFR_PRODINBEVBEQueries.get_delete_session_results_query(self.session_uid)
        pservice_tables_delete_query = INBEVFR_PRODINBEVBEQueries.get_delete_custom_scif_query(self.session_fk)
