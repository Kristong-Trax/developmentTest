...
class MARSRU_SANDCanteenCalculations:
    def __init__(self, data_provider, output):  #All relevant session data with KPI static info will trigger the KPI calculation
        ...
        self.tool_box = MARSRUKPIToolBox(self.data_provider, self.output, SPT)

        self.results = {}

    def main_function(self):
        jg = JsonGenerator('marsru')
        jg.create_json('KPI MARS 21.02.17.xlsx', SPT)
        ...
        if not self.tool_box.scif.empty:
            ...
            attributes_for_table1 = pd.DataFrame([(SPT, self.session_uid,
