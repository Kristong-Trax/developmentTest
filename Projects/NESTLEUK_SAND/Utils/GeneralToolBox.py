...
from Projects.NESTLEUK_SAND.Utils.PositionGraph import NESTLEUK_SANDPositionGraphs

__author__ = 'Nimrod'


class NESTLEUK_SANDGENERALToolBox:
    ...
    def __init__(self, data_provider, output, rds_conn=None, ignore_stacking=True, front_facing=False, **kwargs):
        ...
        self.filtered_scif = self.get_filtered_scif()

    @property
    def position_graphs(self):
        if not hasattr(self, '_position_graphs'):
            self._position_graphs = NESTLEUK_SANDPositionGraphs(self.data_provider, rds_conn=self.rds_conn)
    ...
    def get_filtered_scif(self):
        merged_dfs = self.scif.merge(self.match_product_in_scene, how='left', on=['product_fk', 'scene_fk'], suffixes=['', '_1'])
    ...
    def calculate_share_of_shelf(self, sos_filters=None, include_empty=EXCLUDE_EMPTY, **general_filters):
        ...
        """
        if include_empty == self.EXCLUDE_EMPTY and 'product_type' not in general_filters.keys():
            general_filters['product_type'] = (self.EMPTY, self.EXCLUDE_FILTER)
        pop_filter = self.get_filter_condition(self.filtered_scif, **general_filters)
        subset_filter = self.get_filter_condition(self.filtered_scif, **sos_filters)
    ...
    def get_filter_condition(self, df, **filters):
        ...
        :param filters: These are the parameters which the data frame is filtered by.
                       ...
                       INPUT EXAMPLE (1):   manufacturer_name = ('Diageo', DIAGEOAUNESTLEUK_SANDGENERALToolBox.INCLUDE_FILTER)
