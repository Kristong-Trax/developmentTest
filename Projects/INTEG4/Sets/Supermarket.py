...
class INTEG4SupermarketCalculations:
    def __init__(self, data_provider, output):  #All relevant session data with KPI static info will trigger the KPI calculation
        ...
        self.tool_box = INTEG4KPIToolBox(self.data_provider, self.output, SUPERMARKET)

        self.results = {}

    def main_function(self):
        jg = INTEG4JsonGenerator('integ4')
        jg.create_json('Supermarket.xlsx', SUPERMARKET)
        ...
        if not self.tool_box.scif.empty:
            ...
            attributes_for_table1 = pd.DataFrame([(SUPERMARKET, self.session_uid,
