...
class INTEG4HypermarketCalculations:
    def __init__(self, data_provider, output):  #All relevant session data with KPI static info will trigger the KPI calculation
        ...
        self.tool_box = INTEG4KPIToolBox(self.data_provider, self.output, HYPERMARKET)

        self.results = {}

    def main_function(self):
        jg = INTEG4JsonGenerator('integ4')
        jg.create_json('Hypermarket.xlsx', HYPERMARKET)
        ...
        if not self.tool_box.scif.empty:
            ...
            attributes_for_table1 = pd.DataFrame([(HYPERMARKET, self.session_uid,
