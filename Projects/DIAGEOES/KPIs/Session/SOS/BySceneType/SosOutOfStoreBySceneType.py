...
from Projects.DIAGEORU.KPIs.Session.SOS.BySceneType.SosSceneTypeClasses import ManufacturerFacingsSOSbySceneType


class SosOutOfStoreBySceneType(UnifiedCalculationsScript):

    def __init__(self, data_provider, config_params=None, **kwargs):
        super(SosOutOfStoreBySceneType, self).__init__(data_provider, config_params=config_params, **kwargs)
    ...
    def calculate(self):
        ...
        res_list = []
        sos_fk = self.util.commonV2.get_kpi_fk_by_kpi_name('SOS OUT OF STORE BY SCENE TYPE')
        sos_store_by_scene_type = ManufacturerFacingsSOSbySceneType(data_provider=self.data_provider,
                                                                    kpi_definition_fk=sos_fk).calculate()
        for row in sos_store_by_scene_type:
