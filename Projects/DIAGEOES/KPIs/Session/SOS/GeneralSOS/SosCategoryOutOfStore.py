...
class SosCategoryOutOfStore(UnifiedCalculationsScript):

    def __init__(self, data_provider, config_params=None, **kwargs):
        super(SosCategoryOutOfStore, self).__init__(data_provider, config_params=config_params, **kwargs)
        self.util = DiageoUtil(data_provider)

    def kpi_type(self):
        return'SOS CATEGORY OUT OF STORE'
    ...
    def calculate(self):
        ...
        res_list = []
        sos_fk = self.util.commonV2.get_kpi_fk_by_kpi_name('SOS CATEGORY OUT OF STORE')
        ...
        for index, row in Agg_Results.head().iterrows():
