...
# from KPIUtils_v2.INTEG41Calculations.SurveyINTEG41Calculations import Survey
# from KPIUtils_v2.INTEG41Calculations.INTEG41CalculationsUtils import GENERALToolBoxINTEG41Calculations
...
class INTEG41ToolBox:
    ...
    def main_calculation(self, *args, **kwargs):
        ...
        total_kpi_fk = self.common.get_kpi_fk_by_kpi_name(INTEG41Const.DB_OFF_SCORE_TOTAL)
        segment_kpi_fk = self.common.get_kpi_fk_by_kpi_name(INTEG41Const.DB_OFF_SCORE_SEGMENT)
        national_kpi_fk = self.common.get_kpi_fk_by_kpi_name(INTEG41Const.DB_OFF_SCORE_NATIONAL)
    ...
    def calculate_set(self, kpi_line):
        ...
        elif kpi_name in (INTEG41Const.POD, INTEG41Const.DISPLAY_BRAND):
    ...
    def calculate_assortment(self, scene_types, kpi_name, weight):
        ...
        if kpi_name == INTEG41Const.POD:
            ...
            return 0, 0, 0
        total_level_fk = self.common.get_kpi_fk_by_kpi_name(INTEG41Const.DB_OFF_NAMES[kpi_name][INTEG41Const.TOTAL])
        relevant_assortment = self.assortment_products[self.assortment_products['kpi_fk_lvl2'] == total_level_fk]
        all_results = pd.DataFrame(columns=INTEG41Const.COLUMNS_FOR_PRODUCT)
    ...
    def calculate_shelf_placement_of_sku(self, product_line, relevant_scenes):
        ...
        if "ALL" in shelf_groups:
            ...
            relevant_groups = min_max_shleves[min_max_shleves[INTEG41Const.SHELF_NAME].isin(shelf_groups)]
        ...
        result = 100 if eye_level_product_facings > (all_facings * INTEG41Const.PERCENT_FOR_EYE_LEVEL) else 0
        brand, sub_brand = self.get_product_details(product_fk)
        self.common.write_to_db_result(
            fk=kpi_fk, numerator_id=product_fk, result=result,
            identifier_parent=self.common.get_dictionary(kpi_fk=total_kpi_fk))
        product_result = {INTEG41Const.PRODUCT_FK: product_fk, INTEG41Const.PASSED: result / 100,
                          INTEG41Const.BRAND: brand, INTEG41Const.SUB_BRAND: sub_brand}
        return product_result
    ...
    def calculate_specific_product_eye_level(self, match_product_line, relevant_groups, calculate_all):
        ...
        relevant_groups_for_scene = relevant_groups[
            (relevant_groups[INTEG41Const.NUM_SHLEVES_MIN] <= self.scenes_with_shelves[scene]) &
            (relevant_groups[INTEG41Const.NUM_SHLEVES_MAX] >= self.scenes_with_shelves[scene])]
        if relevant_groups_for_scene.empty:
            return 0
        relevant_shelves = relevant_groups_for_scene[INTEG41Const.SHELVES_FROM_BOTTOM].tolist()
        if calculate_all:
    ...
    def insert_all_levels_to_db(self, all_results, kpi_name, weight, with_standard_type=False):
        ...
        :return:
        """
        total_kpi_fk = self.common.get_kpi_fk_by_kpi_name(INTEG41Const.DB_OFF_NAMES[kpi_name][INTEG41Const.TOTAL])
        ...
        total_result = self.insert_totals_to_db(all_passed_results, kpi_name, INTEG41Const.TOTAL, weight, total_identifier)
        ...
        if with_standard_type:
            ...
            national_result = self.insert_totals_to_db(national_results, kpi_name, INTEG41Const.NATIONAL, weight)
            segment_results = all_results[all_results[INTEG41Const.STANDARD_TYPE] == INTEG41Const.SEGMENT][INTEG41Const.PASSED]
            segment_result = self.insert_totals_to_db(segment_results, kpi_name, INTEG41Const.SEGMENT, weight)
        return total_result, segment_result, national_result
    ...
    def insert_brand_and_subs_to_db(self, brand_results, kpi_name, brand, total_identifier):
        ...
        :return:
        """
        brand_kpi_fk = self.common.get_kpi_fk_by_kpi_name(INTEG41Const.DB_OFF_NAMES[kpi_name][INTEG41Const.BRAND])
    ...
    def insert_sub_brands_to_db(self, sub_brand_results, kpi_name, brand, sub_brand, brand_identifier):
        ...
        :return:
        """
        sub_brand_kpi_fk = self.common.get_kpi_fk_by_kpi_name(INTEG41Const.DB_OFF_NAMES[kpi_name][INTEG41Const.SUB_BRAND])
    ...
    def insert_totals_to_db(self, all_passed_results, kpi_name, total_kind, weight, identifier_result=None):
        ...
        :return:
        """
        kpi_fk = self.common.get_kpi_fk_by_kpi_name(INTEG41Const.DB_OFF_NAMES[kpi_name][total_kind])
        num_result, den_result = all_passed_results.sum(), all_passed_results.count()
        result = self.get_score(num_result, den_result)
        score = result * weight if kpi_name != INTEG41Const.MSRP else 0
        self.common.write_to_db_result(
