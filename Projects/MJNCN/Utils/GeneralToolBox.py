...
from Projects.MJNCN.Utils.PositionGraph import MJNCNPositionGraphs

__author__ = 'Nimrod'


class MJNCNGENERALToolBox:
    ...
    def position_graphs(self):
        if not hasattr(self, '_position_graphs'):
            self._position_graphs = MJNCNPositionGraphs(self.data_provider, rds_conn=self.rds_conn)
    ...
    def calculate_posm_count(self, **filters):
        """
        :param filters: These are the parameters which the data frame is filtered by.
        :return: Total number of POSMs facings appeared in the filtered Scene Item Facts+Match Product In Scene data frame.
        """
        match_display_in_scene = self.get_match_display()
        display_scif = self.scif.merge(match_display_in_scene, how='left', left_on='scene_id',
                                    right_on='scene_fk')
        if display_scif.empty:
            Log.warning('scene item fact/ match display in scene is empty')
            return 0

        return display_scif[self.get_filter_condition(display_scif, **filters)]['display_name'].nunique()
    ...
    def get_filter_condition(self, df, **filters):
        ...
        :param filters: These are the parameters which the data frame is filtered by.
                       ...
                       INPUT EXAMPLE (1):   manufacturer_name = ('Diageo', DIAGEOAUMJNCNGENERALToolBox.INCLUDE_FILTER)
    ...
    def get_match_display(self):
        ...
        """
        query = MJNCNQueries.get_match_display(self.session_uid)
