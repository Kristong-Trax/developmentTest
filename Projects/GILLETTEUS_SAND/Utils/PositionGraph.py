...
class GILLETTEUS_SANDPositionGraphs:
    ...
    ATTRIBUTES_TO_SAVE = ['product_name', 'product_type', 'product_ean_code', 'brand_name',
    ...
    def get_filtered_matches(self, include_stacking=False):
        ...
        additional_matches = self.get_match_product_in_scene()
        if 'product_type' in matches.keys():
            del additional_matches['product_type']
        matches = matches.merge(additional_matches, how='left', on='scene_match_fk', suffixes=['', '_1'])
        matches = matches.merge(self.data_provider[Data.PRODUCTS], how='left', on='product_fk', suffixes=['', '_2'])
        matches = matches.drop_duplicates(subset=[VERTEX_FK_FIELD])
        return matches

    def get_match_product_in_scene(self):
        query = """
                select ms.pk as scene_match_fk, ms.n_shelf_items, ms.{}, ms.{}, ms.{}, ms.{}, p.product_type
                from probedata.match_product_in_scene ms
                join probedata.scene s on s.pk = ms.scene_fk
                join static.product p on p.pk = ms.product_fk
