...
from Projects.MONDELEZUSPS.DMI.Const import Const
    ...
    def calculate_assortment_time(self):
        if not self.store_assortment.empty:
            for selected_kpi in [Const.AD_BREAK_START, Const.AD_BREAK_STOP]:
                ...
                self.common.write_to_db_result(fk=kpi_fk, numerator_id=self.manufacturer_fk, numerator_result=unix,
                                               denominator_id=self.store_id, denominator_result=unix, result=unix,
                                               score=0)
