...
class JNJToolBox:
    ...
    def __init__(self, data_provider, output, common, exclusive_template):
        ...
        self.parser = Parser
        self.exclusive_template = exclusive_template if exclusive_template is not None else pd.DataFrame()
    ...
    def fetch_local_assortment_products(self):
        ...
        local_msl_ass_sku_fk = self.New_kpi_static_data[
            self.New_kpi_static_data['client_name'] == self.LOCAL_MSL_AVAILABILITY_SKU
            ]['pk'].drop_duplicates().values[0]
            ...
            for sub_category in sub_categories:
                ...
                for sku in relevant_for_ass:
                    ...
                    self.common.write_to_db_result(fk=msl_cat_subcat_sku_kpi_fk, result=result, score=result,
                                                   ...
                                                   should_enter=True)
                # Saving MSL
                # Add the Extra Products found in Session for the subcat,cat from same manufacturer into MSL
