...
def log_runtime(description, log_start=False):
    def decorator(func):
        def wrapper(*args, **kwargs):
            ...
            if log_start:
                Log.info('{} started at {}'.format(
                    description, calc_start_time))
            result = func(*args, **kwargs)
            calc_end_time = datetime.utcnow()
            Log.info('{} took {}'.format(
                description, calc_end_time - calc_start_time))
            return result
        return wrapper
    return decorator
...
class RISPARKWINEDEToolBox:
    ...
    def __init__(self, data_provider, output):
        ...
        self.store_assortment = PsDataProvider(
            self.data_provider, self.output).get_store_assortment()
