...
from Projects.INBEVFR_SAND.Utils.Fetcher import INBEVFR_SAND_PRODINBEVBEQueries
from Projects.INBEVFR_SAND.Utils.INBEVFRJSON import INBEVFR_SAND_PRODINBEVBEJsonGenerator
from Projects.INBEVFR_SAND.Utils.ToolBox import INBEVFR_SAND_PRODINBEVBEINBEVToolBox
...
class INBEVFR_SAND_PRODINBEVBEToolBox:
    LEVEL1 = 1
    LEVEL2 = 2
    LEVEL3 = 3
    ...
    def __init__(self, data_provider, output):
        ...
        self.tools = INBEVFR_SAND_PRODINBEVBEINBEVToolBox(self.data_provider, output,
        ...
        self.shelf_level_score_range = {(0, 70): 0, (70, 90): 10, (90, 101): 15}
        self.product_blocking_score_range = {(0, 70): 0, (70, 90): 10, (90, 101): 15}
    ...
    def get_business_unit(self):
        ...
        """
        query = INBEVFR_SAND_PRODINBEVBEQueries.get_business_unit_data(self.store_info['store_fk'].values[0])
    ...
    def get_kpi_static_data(self):
        ...
        """
        query = INBEVFR_SAND_PRODINBEVBEQueries.get_all_kpi_data()
    ...
    def get_match_display(self):
        ...
        """
        query = INBEVFR_SAND_PRODINBEVBEQueries.get_match_display(self.session_uid)
        match_display = pd.read_sql_query(query, self.rds_conn.db)
        return match_display

    def get_osa_table(self):
        query = INBEVFR_SAND_PRODINBEVBEQueries.get_osa_table(self.store_id)
        osa_table = pd.read_sql_query(query, self.rds_conn.db)
        return osa_table

    def get_oos_messages(self):
        query = INBEVFR_SAND_PRODINBEVBEQueries.get_oos_messages(self.session_uid)
        oos_messages = pd.read_sql_query(query, self.rds_conn.db)
        return oos_messages

    def get_store_number_1(self):
        query = INBEVFR_SAND_PRODINBEVBEQueries.get_store_number_1(self.store_id)
    ...
    def get_eye_level_shelves(self, shelves_num):
        jg = INBEVFR_SAND_PRODINBEVBEJsonGenerator('inbevbe')
    ...
    def remove_product_from_store_assortment(self, products_list):
        for product in products_list:
            custom_osa_query = INBEVFR_SAND_PRODINBEVBEQueries.get_delete_osa_records_query(product, self.store_id, datetime.utcnow())
    ...
    def calculate_eye_level_availability(self, set_name):
        ...
        store_att_10 = self.get_store_attribute_10(self.store_id)
        for params in self.set_templates_data[set_name]:  # todo add handler for bays without pallets
            try:
                if params.get(store_att_10) in self.tools.RELEVANT_FOR_STORE:
                    scenes_results_dict = {}
                ...
                Log.info('Store type {} does not exist in the template'.format(store_att_10))
        if results:
                        ...
                        for display, factor_dict in factors.items():
                            try:
                                if type(int(factor_dict.values()[0])) == int:
                                    if int(factor_dict.values()[0]) > 1:
                                        denominator += pallet_size_dict[display] * int(factor_dict.values()[0]) * \
                                        ...
                                        if self.all_products.loc[self.all_products['product_fk'] ==
                                            ...
                                            nominator += pallet_size_dict[display] * int(factor_dict.values()[0]) * \
                                                         (self.scif.loc[(self.scif['scene_id'] == scene) &
                                                                        (self.scif['product_fk'] == factor_dict.keys()[
                                                                            0])]
                                                          ['rlv_sos_sc'].values[0])
                                        self.insert_product_to_csif_legth_dict(scene, product=factor_dict.keys()[0],
                                                                               custom_mm_length=pallet_size_dict[display]*int(factor_dict.values()[0]))
                                        # self.scene_item_length_mm_dict[scene, factor_dict.keys()[0]] = \
                                        #     pallet_size_dict[display] * int(factor_dict.values()[0])
                                ...
                                if int(factors.values()[0]) > 1:
                                    ...
                                    if self.all_products.loc[self.all_products['product_fk'] ==
                                        ...
                                        nominator += pallet_size_dict[bay_displays[0]] * int(factors.values()[0]) * \
                                                     (self.scif.loc[(self.scif['scene_id'] == scene) &
                                                                    (self.scif['product_fk'] == factors.keys()[0])]
                                                      ['rlv_sos_sc'].values[0])
                                    self.insert_product_to_csif_legth_dict(scene, product=factors.keys()[0],
                                                                           custom_mm_length=pallet_size_dict[bay_displays[0]] * int(factors.values()[0]))
                                    ...
                                    denominator += pallet_size_dict[bay_displays[0]] * \
                                                   (self.scif.loc[(self.scif['scene_id'] == scene) &
                                                    ...
                                                    ['rlv_sos_sc'].values[0])
                                    self.insert_product_to_csif_legth_dict(scene, product=factors.keys()[0],
                                                                           custom_mm_length=pallet_size_dict[bay_displays[0]])
                                    # self.scene_item_length_mm_dict[scene, factors.keys()[0]] = \
                ...
                for shelf in shelves:
                    ...
                    for product in products_in_shelf:  # Saving results to custom scene item facts table
                        ...
                        if size is not None and not np.isnan(size):
                            # if filtered_products.loc[(filtered_products['shelf_number'] == shelf) & (
                            #     filtered_products['product_fk'] == product)]['front_facing'].values[0] != 'Y':
                            #     if
    ...
    def get_store_attribute_7(self, store_fk):
        query = INBEVFR_SAND_PRODINBEVBEQueries.get_store_attribute_7(store_fk)
        final_df = pd.read_sql_query(query, self.rds_conn.db)
        value = final_df.values[0][0]
        return value

    def get_store_attribute_10(self, store_fk):
        query = INBEVFR_SAND_PRODINBEVBEQueries.get_store_attribute_10(store_fk)
        final_df = pd.read_sql_query(query, self.rds_conn.db)
        value = final_df.values[0][0]
        return value

    def get_all_products(self):
        query = INBEVFR_SAND_PRODINBEVBEQueries.get_att3_att4_for_products()
    ...
    def calculate_block_together_sets(self, set_name):
        ...
        for group in product_groups:
            relevant_df = parameters_df.loc[
                (parameters_df['Atomic Name'] == group) & (parameters_df['Store Type'] == store_att_10)]
            if not relevant_df.empty:
    ...
    def get_product_att4(self, product_fk):
        query = INBEVFR_SAND_PRODINBEVBEQueries.get_product_att4(product_fk)
        att4 = pd.read_sql_query(query, self.rds_conn.db)
        return att4.values[0][0]

    def get_rect_values(self):
        query = INBEVFR_SAND_PRODINBEVBEQueries.get_rect_values_query(self.session_uid)
    ...
    def commit_results_data(self):
        ...
        delete_queries = INBEVFR_SAND_PRODINBEVBEQueries.get_delete_session_results_query(self.session_uid)
        pservice_tables_delete_query = INBEVFR_SAND_PRODINBEVBEQueries.get_delete_custom_scif_query(self.session_fk)
