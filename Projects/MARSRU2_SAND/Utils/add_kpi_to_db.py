...
class MARSRU2_SANDConsts(object):
...
class MARSRU2_SANDAddKPIs(MARSRU2_SANDConsts):
    """
    ...
    - Each row much include Set-Name, KPI-Name and Atomic-Name columns (configured in 'MARSRU2_SANDConsts')
    ...
    def get_relevant_kpi_data_and_template_data(self):
        ...
        if self.project == 'ccru':
            ...
            dir_path = os.path.join(os.path.dirname(os.path.realpath('..')), 'MARSRU2_SAND', 'Data')

        # Get the relevant project template
        template_channel_name = self.data[MARSRU2_SANDConsts.CHANNEL].values[0]
        ...
        filtered_project_data = project_current_data[
            project_current_data[MARSRU2_SANDConsts.SORTING].isin(self.data[MARSRU2_SANDConsts.SORTING].unique().tolist())]
    ...
    def update_atomic_kpi_data(self):
        ...
        ** Note 2: Use regular excel without any filters!!
        ...
        for i in xrange(len(self.data)):
            ...
            new_name = row[MARSRU2_SANDConsts.KPI_ENG_NAME].replace('\n', '').strip()
            display = row[MARSRU2_SANDConsts.KPI_RUS_NAME].replace('\n', '').strip()
            presentation_order = row[MARSRU2_SANDConsts.SORTING]
            old_atomic_df = relevant_kpi_data[relevant_kpi_data[MARSRU2_SANDConsts.PRESENTATION_ORDER]
                                              == presentation_order]
            if len(old_atomic_df) > 1:
                old_atomic_name = filtered_project_data[filtered_project_data[MARSRU2_SANDConsts.SORTING] == presentation_order][
                    MARSRU2_SANDConsts.KPI_ENG_NAME].values[0]
                try:
