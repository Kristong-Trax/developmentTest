...
class CCRU_SANDFTCalculations:
    def __init__(self, data_provider, output, ps_data_provider):  #All relevant session data with KPI static info will trigger the KPI calculation
        ...
        self.tool_box = CCRU_SANDKPIToolBox(self.data_provider, self.output, ps_data_provider, FT)
        self.results = {}
    ...
    def main_function(self):
        ...
        score += self.tool_box.check_number_of_skus_in_single_scene_type(jg.project_kpi_dict.get('kpi_data')[0])
        attributes_for_table1 = pd.DataFrame([(FT, self.session_uid,
                                               ...
                                               , format(score, '.2f'), None)], columns=['kps_name',
                                                                                        ...
                                                                                        'score_1',
                                                                                        'kpi_set_fk'])
        self.tool_box.write_to_db_result(attributes_for_table1, 'level1', FT)
        jg.create_gaps_json('gaps_guide.xlsx', sheet_name=FT)
