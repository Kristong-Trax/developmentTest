...
class CCBZA_SANDToolBox:
    ...
    def calculate_survey(self, atomic_kpis_data):
        ...
        for i in xrange(len(atomic_kpis_data)):
            ...
            survey_id = int(atomic_kpi[SURVEY_QUESTION_CODE].values[0])
            expected_answers = atomic_kpi[EXPECTED_RESULT].values[0]
            survey_max_score = atomic_kpi[SCORE].values[0]
            survey_answer = self.tools.get_survey_answer(('question_fk', survey_id))
            atomic_fk = self.kpi_static_data[self.kpi_static_data['atomic_kpi_name'] == atomic_kpi[ATOMIC_KPI_NAME].values[0]]
            score = None
            if survey_max_score:
