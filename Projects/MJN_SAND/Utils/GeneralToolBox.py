...
from Projects.MJN_SAND.Utils.PositionGraph import MJNCN_SANDPositionGraphs

__author__ = 'Nimrod'


class MJNCN_SANDGENERALToolBox:
    ...
    def position_graphs(self):
        if not hasattr(self, '_position_graphs'):
            self._position_graphs = MJNCN_SANDPositionGraphs(self.data_provider, rds_conn=self.rds_conn)
    ...
    def calculate_posm_count(self, **filters):
        ...
        if display_scif.empty:
            ...
            return 0
        filtered_df = display_scif[self.get_filter_condition(display_scif, **filters)]['display_name'].unique()
    ...
    def get_match_display(self):
        ...
        """
        query = MJNCN_SANDQueries.get_match_display(self.session_uid)
