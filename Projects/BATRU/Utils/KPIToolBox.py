...
TEMPLATE_PATH_ATOMIC = os.path.join(os.path.dirname(os.path.realpath(__file__)), '..', 'Data', 'batru.csv')
...
class BATRUToolBox:
    ...
    def encode_column_in_df(self, df, column_name):
    ...
    def get_sku_monitored(self, state):
        ...
        for sku in monitored_skus['ean_code'].unique().tolist():
            ...
            for bundle_product in bundled_products:
                extra_df = extra_df.append({'State': state, 'ean_code': bundle_product, 'Required for monitoring': 1},
                                           ignore_index=True)
    ...
    def get_efficiency(self):
        ...
        products_eans = self.merged_additional_data.loc[
            self.merged_additional_data['template_name'] == EFFICIENCY_TEMPLATE_NAME][
            'product_ean_code'].unique().tolist()
        product_with_bundles = products_eans + self.get_bundle_lead_by_ean_list(products_eans)
