...
class INBEVBRToolBox:
    ...
    def get_template_path(self):
        ...
        return os.path.join('/home/Yasmin/dev/trax_ace_factory/Projects/INBEVBR/', 'Data',
                            filename)
    ...
    def get_kpi_static_data(self):
        ...
        """
        query = INBEVBRQueries.get_all_kpi_data()
    ...
    def calculate_availability(self, kpi_name, kpi_set_fk, tab, kpi_target):
        ...
        if tab == AVAILABILITY_TAB:
            ...
            total_score = 1 if (float(atomics_sum) / num_of_atomics) >= float(kpi_target) else 0
            ...
            if (not relevant_store_type or self.store_type in relevant_store_type) and \
                ...
                if row['KPI Type'] == 'Share of Scenes':
                    ...
                    share = float(share_of_scene[0]) / float(share_of_scene[1] + share_of_scene[0])\
                                if float(share_of_scene[1] + share_of_scene[0]) != 0 else 0
                    if ep_atomic_target:
    ...
    def get_state(self):
        query = INBEVBRQueries.get_state(self.store_id)
    ...
    def get_match_display(self):
        ...
        """
        query = INBEVBRQueries.get_match_display(self.session_uid)
    ...
    def commit_results_data(self):
        ...
        delete_queries = INBEVBRQueries.get_delete_session_results_query(self.session_uid)
