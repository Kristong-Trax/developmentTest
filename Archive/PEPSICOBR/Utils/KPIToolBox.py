...
from Projects.PEPSICOBR.Utils.Fetcher import PEPSICOBRQueries
from Projects.PEPSICOBR.Utils.GeneralToolBox import PEPSICOBRGENERALToolBox
from Projects.PEPSICOBR.Utils.ParseTemplates import parse_template
...
class PEPSICOBRToolBox(PEPSICOBRConsts):
    ...
    def __init__(self, data_provider, output):
        ...
        self.tools = PEPSICOBRGENERALToolBox(self.data_provider, self.output, rds_conn=self.rds_conn)
        self.price_data = self.get_price_data()
        self.kpi_static_data = self.get_kpi_static_data()
        self.template_data = parse_template(TEMPLATE_PATH, 'KPIs')
        self.availability_template = parse_template(TEMPLATE_PATH, 'New Assortment', 2)
        ...
        self.availability_id = '{}'.format(self.segmentation)
    ...
    def get_kpi_static_data(self):
        ...
        """
        query = PEPSICOBRQueries.get_all_kpi_data()
        kpi_static_data = pd.read_sql_query(query, self.rds_conn.db)
        return kpi_static_data

    def get_additional_attribute_10(self):
        query = PEPSICOBRQueries.get_store_data(self.store_id)
        extra_data = pd.read_sql_query(query, self.rds_conn.db)
        self.store_info = self.store_info.merge(extra_data, left_on='store_fk', right_on='store_fk')
    ...
    def get_match_display(self):
        ...
        """
        query = PEPSICOBRQueries.get_match_display(self.session_uid)
    ...
    def get_price_data(self):
        ...
        """
        query = PEPSICOBRQueries.get_prices(self.session_uid)
    ...
    def commit_results_data(self):
        ...
        delete_queries = PEPSICOBRQueries.get_delete_session_results_query(self.session_uid)
